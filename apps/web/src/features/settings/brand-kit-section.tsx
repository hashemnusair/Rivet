...
import type { BrandKit, UpdateBrandKitInput } from "@/lib/domain/types";
...
export function BrandKitSection() {
  ...
  const logo = pendingLogo?.previewUrl || brand.logoUrl;
  return (
    <section className="panel max-w-3xl p-5" data-testid="brand-kit-section">
      ...
      <div className="mt-5 grid gap-5 sm:grid-cols-[minmax(0,1fr)_220px]">
        <div className="space-y-4">
          ...
          <Field label="Workspace logo" hint="Reuses the sanitized public gym logo asset. A descriptive alt text is required.">
            {logo ? <div className="mb-2 flex items-center gap-3 rounded-md border border-line bg-sunken p-2"><span role="img" aria-label={pendingLogo?.altText || brand.logoAltText || `${session?.organization.name ?? "Gym"} logo`} className="size-12 rounded-sm bg-cover bg-center" style={{ backgroundImage: `url(${logo})` }} /><span className="min-w-0 flex-1 truncate text-[12px] text-ink-2">{pendingLogo ? pendingLogo.file.name : "Current logo"}</span>{isOwner ? <Button type="button" size="xs" variant="secondary" onClick={() => setForm((current) => ({ ...current, logoAssetId: null }))}>Remove</Button> : null}</div> : null}
            <div className="flex items-center gap-2"><ImagePlus className="size-4 text-ink-3" aria-hidden /><Input aria-label="Upload workspace logo" type="file" accept="image/jpeg,image/png,image/webp" disabled={!isOwner || save.isPending} onChange={selectLogo} /></div>
      ...
      <div className="mt-5 flex justify-end"><Button disabled={!isOwner || !dirty || !normalizeBrandHex(form.primaryColor) || Boolean(pendingLogo && pendingLogo.altText.trim().length < 3)} loading={save.isPending} onClick={() => save.mutate()}><Save /> Save Brand Kit</Button></div>
    </section>
  );
}
