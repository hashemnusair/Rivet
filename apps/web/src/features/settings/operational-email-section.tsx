...
export function OperationalEmailSection() {
  ...
  const requiresReason = enabledKinds.length < (query.data?.enabledKinds.length ?? 0);
  const save = useApiMutation((api) => api.updateOperationalEmailSettings({ enabledKinds, reason: reason.trim() }), { onSuccess: async () => { await invalidate(); setReason(""); toast.success("Member service email preferences saved."); } });
