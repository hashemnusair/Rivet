...
export function GymPublicProfileSection() {
  ...
  const discardAsset = useApiMutation((api, assetId: string) => api.discardDraftMediaAsset(assetId));
  const removeAsset = (kind: "logo" | "cover" | "gallery", assetId: string) => {
    ...
    if (draftAssetIds.includes(assetId)) {
      setDraftAssetIds((current) => current.filter((id) => id !== assetId));
      discardAsset.mutate(assetId);
    }
  };
    ...
    <div className="grid gap-5 xl:grid-cols-[minmax(0,1.2fr)_minmax(340px,.8fr)]">
      <section className="panel p-5">
        ...
        <div className="mt-5 grid gap-4 sm:grid-cols-2">
          <MediaUploadField label="Logo" current={form.logoAssetId ? uploadedAssets[form.logoAssetId] ?? value.logo : undefined} loading={upload.isPending} onRemove={form.logoAssetId ? () => removeAsset("logo", form.logoAssetId!) : undefined} onUpload={(file, altText) => upload.mutate({ ownerType: "gym_logo", file, altText })} />
          <MediaUploadField label="Cover image" current={form.coverAssetId ? uploadedAssets[form.coverAssetId] ?? value.cover : undefined} loading={upload.isPending} onRemove={form.coverAssetId ? () => removeAsset("cover", form.coverAssetId!) : undefined} onUpload={(file, altText) => upload.mutate({ ownerType: "gym_cover", file, altText })} />
          ...
          <div className="sm:col-span-2 rounded-md border border-line p-3"><p className="flex items-center gap-2 text-[12.5px] font-medium"><ImagePlus className="size-4 text-ink-3" /> Gallery</p><p className="mt-1 text-[11px] text-ink-3">Order the photos members see on the public gym page. Save the draft after changing the order.</p>{form.galleryAssetIds.length ? <ol className="mt-3 grid gap-2 sm:grid-cols-2">{form.galleryAssetIds.map((assetId, index) => { const asset = uploadedAssets[assetId] ?? value.gallery.find((item) => item.id === assetId); return <li key={assetId} className="flex items-center gap-2 rounded-md border border-line bg-sunken p-2"><span className="size-12 shrink-0 rounded-sm bg-cover bg-center" role="img" aria-label={asset?.altText ?? `Gallery image ${index + 1}`} style={{ backgroundImage: asset?.url ? `url(${asset.url})` : undefined }} /><span className="min-w-0 flex-1"><span className="block truncate text-[11.5px] font-medium">Image {index + 1}</span><span className="block truncate text-[10px] text-ink-3">{asset?.altText ?? "Pending preview"}</span></span><Button type="button" size="icon" variant="ghost" aria-label={`Move image ${index + 1} earlier`} disabled={index === 0} onClick={() => setForm((current) => { const ids = [...current.galleryAssetIds]; [ids[index - 1], ids[index]] = [ids[index]!, ids[index - 1]!]; return { ...current, galleryAssetIds: ids }; })}>↑</Button><Button type="button" size="icon" variant="ghost" aria-label={`Move image ${index + 1} later`} disabled={index === form.galleryAssetIds.length - 1} onClick={() => setForm((current) => { const ids = [...current.galleryAssetIds]; [ids[index], ids[index + 1]] = [ids[index + 1]!, ids[index]!]; return { ...current, galleryAssetIds: ids }; })}>↓</Button><Button type="button" size="icon" variant="ghost" aria-label={`Remove image ${index + 1}`} onClick={() => removeAsset("gallery", assetId)}>×</Button></li>; })}</ol> : <p className="mt-3 rounded-md border border-dashed border-line-2 px-3 py-4 text-center text-[11.5px] text-ink-3">No gallery images selected.</p>}<MediaUploadField label="Add gallery image" loading={upload.isPending} onUpload={(file, altText) => upload.mutate({ ownerType: "gym_gallery", file, altText })} /></div>
        </div>
...
function MediaUploadField({ label, current, loading, onUpload, onRemove }: { label: string; current?: MediaAsset; loading: boolean; onUpload: (file: File, altText: string) => void; onRemove?: () => void }) {
  const [altText, setAltText] = useState(current?.altText ?? "");
  const [file, setFile] = useState<File>();
  const fieldId = useId();
  useEffect(() => { setAltText(current?.altText ?? ""); }, [current?.id, current?.altText]);
  return (
    <div className="rounded-md border border-line p-3">
      ...
      {current?.url ? <div className="relative mt-3"><div role="img" aria-label={current.altText ?? "Uploaded profile image"} className="h-24 w-full rounded-sm bg-cover bg-center" style={{ backgroundImage: `url(${current.url})` }} />{onRemove ? <Button type="button" size="xs" variant="secondary" className="absolute right-2 top-2" onClick={onRemove}>Remove</Button> : null}</div> : null}
