    ...
    <div className="grid gap-5 xl:grid-cols-[minmax(0,1.2fr)_minmax(340px,.8fr)]">
      ...
      <div className="space-y-5">
        <section className="overflow-hidden rounded-lg border border-line bg-surface">
          ...
          <div className="p-5"><div className="flex items-center gap-3"><span className="size-12 shrink-0 rounded-full border border-line bg-cover bg-center" role="img" aria-label={pendingMedia.logo || form.logoAssetId ? "Gym logo preview" : "Gym logo placeholder"} style={{ backgroundColor: form.accentColor, backgroundImage: logoPreviewUrl ? `url(${logoPreviewUrl})` : undefined }} /><div><p className="font-mono text-[9px] uppercase tracking-[.14em] text-ink-3">{form.category || "Gym"} · {form.audience || "All members"}</p><h3 className="mt-1 font-display text-[25px] font-semibold">{form.shortName || "Gym name"}</h3></div></div><p className="mt-3 text-[13px] leading-relaxed text-ink-2">{form.taglineEn || "Add the gym's public tagline."}</p><div className="mt-4 flex flex-wrap gap-1.5">{amenities.split(",").map((item) => item.trim()).filter(Boolean).map((item) => <span key={item} className="rounded-full border border-line px-2 py-1 text-[10px]">{item}</span>)}</div><div className="mt-5 border-t border-line pt-4 text-[11.5px] text-ink-3">{pendingMedia.logo || pendingMedia.cover ? "Local image preview · save draft to upload" : `${value.trainers.length} published trainer${value.trainers.length === 1 ? "" : "s"} · ${value.ptPackages.length} active PT package${value.ptPackages.length === 1 ? "" : "s"}`}</div></div>
        </section>
...
function MediaUploadField({ label, current, draft, loading, onSelect, onAltTextChange, onRemove }: { label: string; current?: MediaAsset; draft?: PendingMedia; loading: boolean; onSelect: (file: File, altText: string) => void; onAltTextChange?: (altText: string) => void; onRemove?: () => void }) {
  ...
  return (
    <div className="rounded-md border border-line p-3">
      ...
      {draft ? <p className={`mt-2 text-[10px] ${draft.altText.trim().length >= 3 ? "text-ink-3" : "text-red-700"}`}>{draft.altText.trim().length >= 3 ? "Local preview only. Save draft to upload and sanitize this image." : "Add an accessible image description before saving."}</p> : null}
      <p className="mt-2 text-[9.5px] text-ink-3">Choose a JPEG, PNG, or WebP · 5 MB maximum · preview is local until Save draft.</p>
    </div>
  );
}
