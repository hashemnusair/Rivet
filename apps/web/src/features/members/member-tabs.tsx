      ...
      <Dialog open={bookingOpen} onOpenChange={setBookingOpen}>
        <DialogContent className="max-w-xl">
          ...
          <DialogBody className="grid gap-3">
            <label className="grid gap-1 text-[12px] font-medium">Trainer<select aria-label="Trainer" className="h-9 rounded-md border border-line-2 bg-surface px-3 text-[12px]" value={trainerId} onChange={(event) => { setTrainerId(event.target.value); setBranchId(""); }}><option value="">Choose a trainer</option>{experience.trainers.map((trainer) => <option key={trainer.id} value={trainer.id}>{trainer.displayName}</option>)}</select></label>
            {selectedTrainer ? <label className="grid gap-1 text-[12px] font-medium">Branch<select aria-label="Branch" className="h-9 rounded-md border border-line-2 bg-surface px-3 text-[12px]" value={visibleBranchId(session?.branches, selectedBranch) ?? ""} onChange={(event) => setBranchId(event.target.value)}><option value="">Choose a branch</option>{selectedTrainer.branchIds.filter((id) => visibleBranchId(session?.branches, id)).map((id) => <option key={id} value={id}>{session?.branches.find((branch) => branch.id === id)?.name ?? id}</option>)}</select></label> : null}
            <label className="grid gap-1 text-[12px] font-medium">Date<input aria-label="Date" className="h-9 rounded-md border border-line-2 bg-surface px-3 text-[12px]" type="date" min={addDays(todayISODate(), 1)} value={date} onChange={(event) => setDate(event.target.value)} /></label>
            {trainerId && selectedBranch ? <div><p className="mb-2 text-[12px] font-medium">Available times</p>{(slots.isError || slots.isBackgroundError) ? <ErrorState layout="section" title="Available times could not be loaded" onRetry={() => slots.refetch()} /> : slots.isLoading ? <p className="text-[12px] text-ink-3">Loading slots…</p> : slots.data?.length ? <div className="flex flex-wrap gap-2">{slots.data.map((slot) => <Button key={slot.startsAt} size="sm" variant="secondary" loading={book.isPending} onClick={() => book.mutate(slot.startsAt)}>{new Intl.DateTimeFormat("en-JO", { hour: "numeric", minute: "2-digit", timeZone: session?.organization.timezone ?? "Asia/Amman" }).format(new Date(slot.startsAt))}</Button>)}</div> : <p className="text-[12px] text-ink-3">No open 60-minute slots on this date.</p>}</div> : <p className="text-[12px] text-ink-3">Choose a trainer and date to load available times.</p>}
          </DialogBody>
