...
export function InventoryTab({ branchId, branchLabel, branches, currency, writeEnabled, products, suppliers, inventory, alerts, loading, error, onRetry, mutations, onSell }: { branchId?: string; branchLabel: string; branches: Array<{ id: string; name: string }>; currency: string; writeEnabled: boolean; products: Product[]; suppliers: Supplier[]; inventory: InventoryBalance[]; alerts: LowStockAlert[]; loading: boolean; error?: unknown; onRetry: () => void; mutations: OperationsMutations; onSell?: (productId: string) => void }) {
  ...
  const visibleProducts = useMemo(() => {
    ...
    return products.filter((product) => (!term || `${product.name} ${product.sku}`.toLowerCase().includes(term)) && (!attentionOnly || alertProductIds.has(product.id)))
  ...
  return (
    <div className="space-y-4" data-testid="operations-inventory">
      <section className="panel overflow-hidden">
        ...
        <div><table className="block w-full text-start md:table"><caption className="sr-only">Available inventory</caption><thead className="sr-only border-b border-line bg-sunken/40 text-[12px] text-ink-3 md:not-sr-only md:table-header-group"><tr><th className="px-4 py-2.5 font-medium">Item</th><th className="px-4 py-2.5 text-end font-medium">Available</th><th className="px-4 py-2.5 font-medium">Status</th><th className="px-4 py-2.5 text-end font-medium">Selling price</th><th className="px-4 py-2.5 text-end font-medium">Actions</th></tr></thead><tbody className="block divide-y divide-line md:table-row-group">{products.length === 0 ? <tr><td colSpan={5}><EmptyState compact title="No stock items yet" description="Add an item to start tracking what is available." className="m-4" /></td></tr> : visibleProducts.length === 0 ? <tr><td colSpan={5}><EmptyState compact title="No matching items" description={attentionOnly ? "No items match this search and replenishment filter. Clear the search or show all stock." : "Try a different name or SKU."} className="m-4" /></td></tr> : visibleProducts.map((product) => { const rows = inventoryByProduct.get(product.id) ?? []; const productAlerts = alerts.filter((alert) => alert.productId === product.id); const selectedRow = rows.find((row) => row.branchId === branchId); const available = selectedRow?.availableQuantity ?? 0; const totalAvailable = rows.reduce((sum, row) => sum + row.availableQuantity, 0); const low = selectedRow ? available <= product.reorderPoint : rows.some((row) => row.availableQuantity <= product.reorderPoint); const needsReplenishment = alertProductIds.has(product.id); const alertBranches = [...new Set(productAlerts.map((alert) => branches.find((branch) => branch.id === alert.branchId)?.name ?? alert.branchId))].join(", "); const sellable = Boolean(onSell && branchId && available > 0 && hasSellableRetailPrice(product, currency)); return <tr key={product.id} className="grid grid-cols-2 items-start gap-y-2 px-4 py-4 text-[13.5px] md:table-row md:p-0"><td className="col-span-2 md:px-4 md:py-3"><span className="font-medium">{product.name}</span><span className="block text-[12px] text-ink-3">{product.sku} · {product.unit}</span></td><td className={cn("tabular-nums md:px-4 md:py-3 md:text-end", needsReplenishment ? "text-warning-deep" : "text-ink")} dir="ltr"><span className="me-2 text-[12px] text-ink-3 md:hidden">Available</span>{branchId ? available : <><span>Total {totalAvailable}</span>{rows.length > 0 ? <span className="mt-1 block text-[12px] text-ink-3">{rows.map((row) => `${branches.find((branch) => branch.id === row.branchId)?.name ?? row.branchId}: ${row.availableQuantity}`).join(" · ")}</span> : null}</>}</td><td className="md:px-4 md:py-3">{needsReplenishment ? <Badge variant="warning" dot>{branchId ? (low ? "Low stock" : "Replenish soon") : `${low ? "Low stock" : "Replenish soon"} · ${alertBranches}`}</Badge> : <Badge variant="success" dot>Available</Badge>}</td><td className="tabular-nums md:px-4 md:py-3 md:text-end" dir="ltr"><span className="me-2 text-[12px] text-ink-3 md:hidden">Price</span>{product.retailPrice ? <MoneyText money={product.retailPrice} /> : <span className="text-ink-3">Not set</span>}</td><td className="col-span-2 border-t border-line pt-2 md:border-0 md:px-4 md:py-3 md:text-end"><div className="flex flex-wrap items-center gap-1 md:justify-end">{sellable ? <Button size="xs" variant="secondary" aria-label={"Sell " + product.name} onClick={() => onSell!(product.id)}><ShoppingBag /> Sell</Button> : null}{writeEnabled && branchId ? <Button size="xs" variant={needsReplenishment ? "primary" : "ghost"} aria-label={"Reorder " + product.name} onClick={() => setOrderForm({ defaultProductId: product.id })}><PackagePlus /> Reorder</Button> : null}{writeEnabled ? <Button size="icon" variant="ghost" aria-label={"Edit " + product.name} onClick={() => { setEditingProduct(product); setProductForm(true); }} disabled={!branchId}><Pencil /></Button> : null}</div></td></tr>; })}</tbody></table></div>
