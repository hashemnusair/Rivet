...
function ProductForm({ currency, suppliers, pending, onCancel, onSubmit }: { currency: string; suppliers: Supplier[]; pending: boolean; onCancel: () => void; onSubmit: (input: UpsertProductInput) => void }) {
  const [form, setForm] = useState({ sku: "", name: "", unit: "each", reorderPoint: "", targetLevel: "", leadTime: "", supplierId: "", cost: "" });
  return (
    <FormPanel title="Add stock item" description="Set the reorder threshold used by low-stock detection." onCancel={onCancel}>
      <form className="grid gap-3 sm:grid-cols-2" onSubmit={(event) => { event.preventDefault(); onSubmit({ sku: form.sku, name: form.name, unit: form.unit as UpsertProductInput["unit"], reorderPoint: Number(form.reorderPoint), targetLevel: Number(form.targetLevel), supplierLeadTimeDays: Number(form.leadTime), preferredSupplierId: form.supplierId || undefined, defaultUnitCost: minorValue(form.cost, currency) }); }}>
        ...
        <div className="flex justify-end gap-2 sm:col-span-2"><Button type="submit" loading={pending}><PackagePlus /> Save item</Button></div>
      </form>
    </FormPanel>
  );
}

function SupplierForm({ defaultBranchId, branches, pending, onCancel, onSubmit }: { defaultBranchId?: string; branches: Array<{ id: string; name: string }>; pending: boolean; onCancel: () => void; onSubmit: (input: UpsertSupplierInput) => void }) {
  const [form, setForm] = useState({ name: "", contactName: "", email: "", phone: "", terms: "", leadTime: "", branchIds: defaultBranchId ? [defaultBranchId] : [] as string[] });
  return (
    <FormPanel title="Add supplier" description="Keep supplier contacts and branch coverage in one place." onCancel={onCancel}>
      <form className="grid gap-3 sm:grid-cols-2" onSubmit={(event) => { event.preventDefault(); onSubmit({ name: form.name, contactName: form.contactName || undefined, email: form.email || undefined, phone: form.phone || undefined, terms: form.terms || undefined, leadTimeDays: form.leadTime ? Number(form.leadTime) : undefined, branchIds: form.branchIds }); }}>
        ...
        <div className="flex justify-end gap-2 sm:col-span-2"><Button type="submit" loading={pending}><Store /> Save supplier</Button></div>
      </form>
    </FormPanel>
  );
}

function MovementForm({ currency, products, branchId, pending, onCancel, onSubmit }: { currency: string; products: Product[]; branchId?: string; pending: boolean; onCancel: () => void; onSubmit: (input: { branchId: string; productId: string; type: "receive" | "sale" | "consumption" | "adjustment" | "return" | "transfer_in" | "transfer_out" | "waste"; quantity: number; unitCost?: ReturnType<typeof money>; reason?: string; idempotencyKey: string }) => void }) {
  const [form, setForm] = useState({ productId: products[0]?.id ?? "", type: "receive", quantity: "", unitCost: "", reason: "" });
  return (
    <FormPanel title="Record stock movement" description="Every movement is idempotent and leaves an audit trail." onCancel={onCancel}>
      <form className="grid gap-3 sm:grid-cols-2" onSubmit={(event) => { event.preventDefault(); if (!branchId) return; onSubmit({ branchId, productId: form.productId, type: form.type as "receive", quantity: Number(form.quantity), unitCost: minorValue(form.unitCost, currency), reason: form.reason || undefined, idempotencyKey: newKey("stock") }); }}>
...
function InventoryTab({ branchId, branchLabel, branches, currency, writeEnabled, products, suppliers, inventory, alerts, orders, loading, error, onRetry, mutations }: { branchId?: string; branchLabel: string; branches: Array<{ id: string; name: string }>; currency: string; writeEnabled: boolean; products: Product[]; suppliers: Supplier[]; inventory: InventoryBalance[]; alerts: LowStockAlert[]; orders: PurchaseOrder[]; loading: boolean; error?: unknown; onRetry: () => void; mutations: OperationsMutations }) {
  ...
  return (
    <div className="space-y-4" data-testid="operations-inventory">
      ...
      {productForm ? <ProductForm currency={currency} suppliers={suppliers} pending={mutations.product.isPending} onCancel={() => setProductForm(false)} onSubmit={(input) => mutations.product.mutate(input, { onSuccess: () => setProductForm(false) })} /> : null}
      {supplierForm ? <SupplierForm defaultBranchId={branchId} branches={branches} pending={mutations.supplier.isPending} onCancel={() => setSupplierForm(false)} onSubmit={(input) => mutations.supplier.mutate(input, { onSuccess: () => setSupplierForm(false) })} /> : null}
...
function useOperationsMutations(invalidate: ReturnType<typeof useInvalidate>): OperationsMutations {
  ...
  return { product, supplier, movement, purchaseOrder, approveOrder, receiveOrder, notifySupplier, refreshAlerts, dismissAlert, facility, asset, issue, workOrder } as OperationsMutations;
}
...
export function OperationsCommandCenter() {
  ...
  const dataError = productQuery.error ?? supplierQuery.error ?? inventoryQuery.error ?? alertQuery.error ?? ordersQuery.error ?? zonesQuery.error ?? facilityQuery.error ?? assetQuery.error ?? issueQuery.error ?? workOrderQuery.error;
  const retryAll = () => { void Promise.all([productQuery.refetch(), supplierQuery.refetch(), inventoryQuery.refetch(), alertQuery.refetch(), ordersQuery.refetch(), zonesQuery.refetch(), facilityQuery.refetch(), assetQuery.refetch(), issueQuery.refetch(), workOrderQuery.refetch()]); };
  const loading = [productQuery, supplierQuery, inventoryQuery, alertQuery, ordersQuery, zonesQuery, facilityQuery, assetQuery, issueQuery, workOrderQuery].some((query) => query.isLoading);
  ...
  return <div className="space-y-4" data-testid="operations-command-center"><PageHeader eyebrow="Operations" title="Daily operations" description={`Keep ${branchLabel.toLowerCase()} ready: stock, facilities, suppliers, and equipment decisions in one queue.`} actions={<div className="flex items-center gap-2 rounded-md border border-line bg-surface px-2.5 py-1.5 text-[11.5px] text-ink-2"><span className="size-1.5 rounded-full bg-success" aria-hidden />{branchLabel}</div>} />{dataError && !loading ? <div className="rounded-md border border-warning/40 bg-warning-bg px-3 py-2 text-[12px] text-warning-deep" role="status">Some operational panels could not refresh. Showing the records that loaded. <button type="button" className="font-medium underline" onClick={retryAll}>Retry all</button></div> : null}<Tabs value={tab} onValueChange={(value) => setTab(value as OperationsTab)}><TabsList className="max-w-full overflow-x-auto"><TabsTrigger value="inventory"><Boxes className="size-3.5" /> Inventory & suppliers</TabsTrigger><TabsTrigger value="facilities"><ClipboardCheck className="size-3.5" /> Facilities</TabsTrigger><TabsTrigger value="equipment"><Wrench className="size-3.5" /> Equipment</TabsTrigger></TabsList><TabsContent value="inventory"><InventoryTab branchId={actionBranchId} branchLabel={branchLabel} branches={session?.branches ?? []} currency={currency} writeEnabled={writeEnabled} products={products} suppliers={suppliers} inventory={inventory} alerts={alerts} orders={orders} loading={loading} error={dataError} onRetry={retryAll} mutations={mutations} /></TabsContent><TabsContent value="facilities"><FacilitiesTab branchId={actionBranchId} writeEnabled={writeEnabled} zones={zones} tasks={tasks} loading={loading} error={dataError} onRetry={retryAll} mutations={mutations} /></TabsContent><TabsContent value="equipment"><EquipmentTab branchId={actionBranchId} currency={currency} writeEnabled={writeEnabled} zones={zones} assets={assets} issues={issues} workOrders={workOrders} loading={loading} error={dataError} onRetry={retryAll} mutations={mutations} /></TabsContent></Tabs></div>;
}
