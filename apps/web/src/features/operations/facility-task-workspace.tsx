...
import { CheckCircle2, ClipboardCheck, Copy, Download, Flag, Play, Plus, QrCode, Sparkles } from "lucide-react";
...
const TASK_PRESETS: Array<{ label: string; title: string; kind: FacilityTaskKind; severity: FacilityTaskSeverity }> = [
  ...
  { label: "Inspect area", title: "Area inspection", kind: "inspection", severity: "medium" },
  { label: "Report incident", title: "Facility incident", kind: "incident", severity: "high" },
];
  ...
  return (
    <Dialog open onOpenChange={(open) => { if (!open) onClose(); }}>
      <DialogContent className="max-w-xl">
        <DialogHeader>
          <DialogTitle>{editing ? "Edit facility task" : "Add facility task"}</DialogTitle>
  ...
  return (
    <Dialog open onOpenChange={(open) => { if (!open) onClose(); }}>
      <DialogContent className="max-w-md">
        <DialogHeader><DialogTitle>Area task QR</DialogTitle><DialogDescription>Place this inside the area. Signed-in staff scan it to report cleaning, inspection, or incident work with the location already selected.</DialogDescription></DialogHeader>
        <DialogBody className="space-y-4">
          <Field label="Area"><Select value={zoneId} onValueChange={(value) => { setZoneId(value); setCopied(false); }}><SelectTrigger aria-label="QR area"><SelectValue /></SelectTrigger><SelectContent>{zones.map((item) => <SelectItem key={item.id} value={item.id}>{item.name}</SelectItem>)}</SelectContent></Select></Field>
          ...
          <div className="rounded-md border border-line bg-sunken/50 px-3 py-2 text-[11.5px] leading-5 text-ink-2"><strong>{zone?.name ?? "Area"}</strong><br />Authentication is still required. The QR is a shortcut, not a public write endpoint.</div>
        </DialogBody>
...
export function FacilityTaskWorkspace({ branchId, zones, writeEnabled }: { branchId?: string; zones: Zone[]; writeEnabled: boolean }) {
  ...
  const mutation = useApiMutation((api, input: UpsertFacilityTaskInput) => api.upsertFacilityTask(input), { onSuccess: async () => { await invalidate([qk.operations()]); }, successMessage: "Facility work list updated." });
  ...
  const visibleTasks = showHistory ? tasks : activeTasks;
  ...
  if (!branchId) return <StatePanel icon={ClipboardCheck} title="Choose a branch first" description="Facility work belongs to a specific branch and area. Choose a branch above to see its work list and QR shortcuts." className="mt-2" />;
  ...
  if (tasksQuery.isError) return <QueryErrorState error={tasksQuery.error} onRetry={() => tasksQuery.refetch()} forbiddenDescription="Your role can’t read facility work for this branch." />;
    ...
    <div className="space-y-4" data-testid="operations-facilities">
      ...
      <section className="panel overflow-hidden">
        <div className="flex flex-wrap items-start justify-between gap-3 border-b border-line px-4 py-3.5">
          <div className="flex min-w-0 items-start gap-2.5"><span className="mt-0.5 flex size-7 shrink-0 items-center justify-center rounded-md bg-sunken"><ClipboardCheck className="size-3.5 text-ink-2" aria-hidden /></span><div><h2 className="text-[13px] font-semibold text-ink">Facility work list</h2><p className="mt-0.5 text-[11.5px] text-ink-3">The urgent work is first. Start and complete common tasks without opening a form.</p></div></div>
          <div className="flex flex-wrap gap-2"><Button size="sm" variant="secondary" onClick={() => setShowHistory((current) => !current)}>{showHistory ? "Hide history" : "Show history"}</Button>{writeEnabled ? <><Button size="sm" variant="secondary" onClick={() => setQrDialog(true)} disabled={zones.length === 0}><QrCode /> Area QR</Button><Button size="sm" onClick={() => setTaskDialog("new")} disabled={zones.length === 0}><Plus /> New task</Button></> : null}</div>
        </div>
        {!writeEnabled ? <div className="border-b border-line bg-sunken/50 px-4 py-2 text-[12px] text-ink-2">You have read-only access. Managers can create and update facility work.</div> : null}
        {zones.length === 0 ? <EmptyState title="Add an area first" description="Create branch areas in Settings before assigning facility work or printing QR shortcuts." className="m-4" /> : visibleTasks.length === 0 ? <EmptyState title={showHistory ? "No facility task history" : "No open facility work"} description={showHistory ? "Tasks created for this branch will appear here." : "Everything is clear. Scan an area QR or add a task when work comes up."} className="m-4" /> : <div className="divide-y divide-line">{visibleTasks.map((task) => <article key={task.id} className={cn("grid gap-3 p-4 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center", task.severity === "critical" && ACTIVE_STATUSES.has(task.status) && "bg-danger-bg/20")}><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><Badge variant={statusVariant(task.status)} dot>{task.status.replaceAll("_", " ")}</Badge><Badge variant={task.severity === "critical" ? "danger" : task.severity === "high" ? "warning" : "neutral"}>{task.severity}</Badge><span className="text-[11px] text-ink-3">{task.kind} · {task.zoneName}</span></div><h3 className="mt-2 text-[13px] font-medium text-ink">{task.title}</h3>{task.notes ? <p className="mt-1 text-[11.5px] leading-5 text-ink-2">{task.notes}</p> : null}<p className="mt-1 text-[11px] text-ink-3">{task.dueAt ? <>Due <DateTimeText iso={task.dueAt} /></> : <>Updated <DateTimeText iso={task.updatedAt} /></>}</p></div>{writeEnabled ? <div className="flex flex-wrap items-center gap-1.5 sm:justify-end">{task.status === "open" ? <Button size="xs" variant="secondary" onClick={() => mutation.mutate(taskUpdate(task, "in_progress"))} loading={mutation.isPending}><Play /> Start</Button> : null}{["open", "in_progress", "blocked"].includes(task.status) ? <Button size="xs" onClick={() => mutation.mutate(taskUpdate(task, "completed"))} loading={mutation.isPending}><CheckCircle2 /> Complete</Button> : null}{["open", "in_progress"].includes(task.status) ? <Button size="xs" variant="ghost" onClick={() => mutation.mutate(taskUpdate(task, "blocked"))} loading={mutation.isPending}><Flag /> Block</Button> : null}<Button size="xs" variant="ghost" onClick={() => setTaskDialog(task)}>Edit</Button></div> : null}</article>)}</div>}
      </section>
