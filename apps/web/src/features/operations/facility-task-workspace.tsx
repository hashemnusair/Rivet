    ...
    <Dialog open onOpenChange={(open) => { if (!open) onClose(); }}>
      <DialogContent className="max-w-md">
        ...
        <DialogBody className="space-y-4">
          ...
          <div className="rounded-md border border-line bg-sunken/50 px-3 py-2 text-[11.5px] leading-5 text-ink-2"><strong>{zone?.name ?? "Gym space"}</strong><br />Staff still sign in before creating a task. The QR only saves them from finding and selecting this location.</div>
        </DialogBody>
...
export function FacilityTaskWorkspace({ branchId, zones, writeEnabled }: { branchId?: string; zones: Zone[]; writeEnabled: boolean }) {
  ...
  if (tasksQuery.isError) return <QueryErrorState error={tasksQuery.error} onRetry={() => tasksQuery.refetch()} forbiddenDescription="Your role can’t read maintenance work for this branch." />;
