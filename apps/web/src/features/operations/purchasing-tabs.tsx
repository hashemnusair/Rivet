...
export function PurchaseOrdersTab({ branchId, currency, writeEnabled, products, suppliers, orders, loading, error, onRetry, mutations, highlightOrderId }: { branchId?: string; currency: string; writeEnabled: boolean; products: Product[]; suppliers: Supplier[]; orders: PurchaseOrder[]; loading: boolean; error?: unknown; onRetry: () => void; mutations: OperationsMutations; highlightOrderId?: string }) {
  const [filter, setFilter] = useState<OrderFilter>(() => (highlightOrderId && orders.some((order) => order.id === highlightOrderId && ["received", "cancelled"].includes(order.status)) ? "all" : "open"));
  ...
  if (error) return <QueryErrorState error={error} onRetry={onRetry} forbiddenDescription="Your role can’t read purchase orders for this workspace." />;
  return (
    <div className="space-y-4" data-testid="operations-orders">
      {!writeEnabled ? <ReadOnlyNotice /> : null}
      <section className="panel overflow-hidden">
        <SectionHeader icon={ShoppingCart} title="Purchase orders" description="Approve a draft to reserve stock, then receive it when it arrives. A received supplier order becomes a payable." actions={<div className="flex flex-wrap items-center gap-2"><div className="flex rounded-md border border-line-2 bg-sunken/50 p-0.5" role="group" aria-label="Order status">{ORDER_FILTERS.map((entry) => <button key={entry.value} type="button" aria-pressed={filter === entry.value} className={cn("rounded px-2.5 py-1.5 text-[12px] transition-colors", filter === entry.value ? "bg-surface font-medium text-ink shadow-sm" : "text-ink-3 hover:text-ink")} onClick={() => setFilter(entry.value)}>{entry.label}</button>)}</div><Button asChild size="sm" variant="secondary"><Link href="/operations?tab=payables"><WalletCards /> Payables</Link></Button>{writeEnabled ? <Button size="sm" onClick={() => setOrderForm(true)} disabled={!branchId}><Plus /> New purchase order</Button> : null}</div>} />
...
export function SuppliersTab({ branchId, branches, writeEnabled, suppliers, loading, error, onRetry, mutations }: { branchId?: string; branches: Array<{ id: string; name: string }>; writeEnabled: boolean; suppliers: Supplier[]; loading: boolean; error?: unknown; onRetry: () => void; mutations: OperationsMutations }) {
  ...
  if (loading) return <LoadingGrid />;
  if (error) return <QueryErrorState error={error} onRetry={onRetry} forbiddenDescription="Your role can’t read suppliers for this workspace." />;
  return (
            ...
            {suppliers.map((supplier) => (
              <div key={supplier.id} className="flex flex-wrap items-center gap-3 p-4" data-testid="supplier-row">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-0.5 truncate text-[11.5px] text-ink-3">{supplier.contactName ?? "No contact"} · {supplier.email ?? supplier.phone ?? "No contact channel"}{supplier.terms ? ` · ${supplier.terms}` : ""}</p>
