...
export function ManagerDashboard() {
  ...
  return <div className="space-y-5"><PageHeader eyebrow={formatDate(today)} title={`Operations, ${session?.user.name.split(" ")[0] ?? "manager"}`} description={branchId ? session?.branches.find((branch) => branch.id === branchId)?.name ?? "Accessible branch" : "All accessible branches"} actions={<Button asChild><Link href="/reception">Open reception <ArrowRight /></Link></Button>} />
    <section className="panel grid grid-cols-2 divide-line sm:grid-cols-3 sm:divide-x lg:grid-cols-6"><Metric label="Collected today" loading={loading}><MoneyText money={data?.kpis.revenueToday ?? money(0)} /></Metric><Metric label="Check-ins today" loading={loading}>{data?.kpis.checkInsToday ?? 0}</Metric><Metric label="Pending approvals" loading={loading} warning={Boolean(approvals.data?.length)}>{approvals.data?.length ?? 0}</Metric><Metric label="Shift variances" loading={loading} warning={varianceShifts.length > 0}>{varianceShifts.length}</Metric><Metric label="Overdue follow-ups" loading={loading} warning={overdueTasks.length > 0}>{overdueTasks.length}</Metric><Metric label="Renewals queue" loading={loading} warning={expiring > 0}>{expiring}</Metric></section>
    <div className="grid gap-5 xl:grid-cols-[1.2fr_.8fr]"><section className="panel overflow-hidden"><header className="flex items-center justify-between border-b border-line px-4 py-3"><h2 className="text-[13px] font-semibold">Operational attention</h2><Button asChild variant="ghost" size="sm"><Link href="/audit">Audit trail <ArrowRight /></Link></Button></header><div className="divide-y divide-line">{data?.alerts.slice(0, 5).map((alert) => <Link key={alert.id} href={alert.href} className="flex items-start gap-3 px-4 py-3 hover:bg-sunken"><AlertTriangle className={alert.severity === "critical" ? "mt-0.5 size-4 text-danger" : "mt-0.5 size-4 text-warning"} /><span className="min-w-0"><span className="block text-[12.5px] font-medium">{alert.title}</span><span className="mt-0.5 block text-[10.5px] text-ink-3">{alert.detail}</span></span></Link>)}{(data?.alerts.length ?? 0) === 0 ? <div className="px-5 py-12 text-center"><CheckCircle2 className="mx-auto size-5 text-success" /><p className="mt-3 text-[12px] font-medium">No operational alerts</p><p className="mt-1 text-[10.5px] text-ink-3">This reflects the current persisted records.</p></div> : null}</div></section><section className="panel overflow-hidden"><header className="border-b border-line px-4 py-3"><h2 className="text-[13px] font-semibold">Queues</h2></header><div className="grid divide-y divide-line"><Queue href="/audit" icon={<CheckCircle2 />} label="Approvals awaiting review" value={approvals.data?.length ?? 0} /><Queue href="/payments/shifts" icon={<AlertTriangle />} label="Cash variances awaiting review" value={varianceShifts.length} /><Queue href="/crm/queues" icon={<Clock3 />} label="Overdue CRM tasks" value={overdueTasks.length} /><Queue href="/memberships" icon={<Users />} label="Expiring memberships" value={expiring} /></div></section></div>
  </div>;
}
