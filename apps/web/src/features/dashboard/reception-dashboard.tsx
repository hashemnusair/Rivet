...
export function ReceptionDashboard() {
  ...
  return <div className="space-y-5"><PageHeader sectionLabel={branchName} title={`Front desk, ${session?.user.name.split(" ")[0] ?? "team"}`} description="Live entry, drawer, arrivals, and balance facts for this branch." actions={<Button asChild><Link href="/reception"><ShieldCheck /> Open reception console</Link></Button>} />
    <section className="panel grid grid-cols-2 divide-line sm:grid-cols-5 sm:divide-x"><Metric label="Check-ins today" loading={loading}>{occupancy.data?.checkInsToday ?? 0}</Metric><Metric label="Peak hour" loading={loading}>{occupancy.data?.peakHour ?? "—"}</Metric><Metric label="Open shift" loading={loading}>{shift.data ? "Open" : "None"}</Metric><Metric label="Expected cash" loading={loading}><MoneyText money={expectedCash} /></Metric><Metric label="Outstanding members" loading={loading} warning={Boolean(outstanding.data?.totalItems)}>{outstanding.data?.totalItems ?? 0}</Metric></section>
    <div className="grid gap-5 xl:grid-cols-[1.15fr_.85fr]"><TodayQueue data={dashboard.data?.todayQueue} loading={dashboard.isLoading} /><section className="panel overflow-hidden"><header className="flex items-center justify-between border-b border-line px-4 py-3"><h2 className="text-[13px] font-semibold">Today&apos;s check-in log</h2><Button asChild variant="ghost" size="sm"><Link href="/reception">Entry console <ArrowRight /></Link></Button></header>{(checkIns.data?.items.length ?? 0) === 0 ? <p className="px-5 py-12 text-center text-[12px] text-ink-3">No check-ins recorded today for this branch.</p> : <div className="divide-y divide-line">{checkIns.data?.items.map((checkIn) => <div key={checkIn.id} className="flex items-center gap-3 px-4 py-3"><span className={checkIn.decision === "allowed" ? "size-2 rounded-full bg-success" : "size-2 rounded-full bg-warning"} /><span className="min-w-0 flex-1"><span className="block truncate text-[12px] font-medium">{checkIn.memberName}</span><span className="block text-[10.5px] capitalize text-ink-3">{checkIn.decision.replaceAll("_", " ")}</span></span><time className="font-mono text-[10.5px] text-ink-3">{new Intl.DateTimeFormat("en-JO", { hour: "numeric", minute: "2-digit" }).format(Date.parse(checkIn.occurredAt))}</time></div>)}</div>}</section></div>
    <section className="panel overflow-hidden"><header className="border-b border-line px-4 py-3"><h2 className="text-[13px] font-semibold">Quick actions</h2></header><div className="grid divide-y divide-line sm:grid-cols-2 sm:divide-x sm:divide-y-0 xl:grid-cols-4"><Action href="/reception" icon={<Search />} label="Search member or scan QR" detail="Check entry verdict and check in" /><Action href="/payments" icon={<Banknote />} label="Collect or review payment" detail={`${outstanding.data?.totalItems ?? 0} members with outstanding balances`} /><Action href="/crm/pipeline" icon={<Users />} label="Trial follow-up queue" detail={`${trialLeads.data?.totalItems ?? 0} active trial leads`} /><Action href="/payments/shifts" icon={<Banknote />} label={shift.data ? "Review current cash shift" : "Open a cash shift"} detail={shift.data ? `Opened by ${shift.data.shift.openedByName}` : "Required before recording cash"} /></div></section>
  </div>;
}
