...
export function ReceptionDashboard() {
  ...
  const branchName = session?.branches.find((branch) => branch.id === branchId)?.name ?? t("dashboard.reception.yourBranch");
  ...
  return <div className="space-y-5"><PageHeader eyebrow={branchName} title={`Front desk, ${session?.user.name.split(" ")[0] ?? "team"}`} description={t("dashboard.reception.liveFacts")} actions={<Button asChild><Link href="/reception"><ShieldCheck /> {t("dashboard.reception.openConsole")}</Link></Button>} />
    <section className="panel grid grid-cols-2 divide-line sm:grid-cols-5 sm:divide-x"><Metric label={t("dashboard.reception.checkInsToday")} loading={loading}>{occupancy.data?.checkInsToday ?? 0}</Metric><Metric label={t("dashboard.reception.peakHour")} loading={loading}>{occupancy.data?.peakHour ?? "—"}</Metric><Metric label={t("dashboard.reception.openShift")} loading={loading}>{shift.data ? "Open" : "None"}</Metric><Metric label={t("dashboard.reception.expectedCash")} loading={loading}><MoneyText money={expectedCash} /></Metric><Metric label={t("dashboard.reception.outstandingMembers")} loading={loading} warning={Boolean(outstanding.data?.totalItems)}>{outstanding.data?.totalItems ?? 0}</Metric></section>
    <div className="grid gap-5 xl:grid-cols-[1.15fr_.85fr]"><section className="panel overflow-hidden"><header className="flex items-center justify-between border-b border-line px-4 py-3"><h2 className="text-[13px] font-semibold">{t("dashboard.reception.todayLog")}</h2><Button asChild variant="ghost" size="sm"><Link href="/reception">{t("dashboard.reception.entryConsole")} <ArrowRight /></Link></Button></header>{(checkIns.data?.items.length ?? 0) === 0 ? <p className="px-5 py-12 text-center text-[12px] text-ink-3">{t("dashboard.reception.noCheckIns")}</p> : <div className="divide-y divide-line">{checkIns.data?.items.map((checkIn) => <div key={checkIn.id} className="flex items-center gap-3 px-4 py-3"><span className={checkIn.decision === "allowed" ? "size-2 rounded-full bg-success" : "size-2 rounded-full bg-warning"} /><span className="min-w-0 flex-1"><span className="block truncate text-[12px] font-medium">{checkIn.memberName}</span><span className="block text-[9.5px] capitalize text-ink-3">{checkIn.decision.replaceAll("_", " ")}</span></span><time className="font-mono text-[9px] text-ink-3">{new Intl.DateTimeFormat("en-JO", { hour: "numeric", minute: "2-digit" }).format(Date.parse(checkIn.occurredAt))}</time></div>)}</div>}</section><section className="panel overflow-hidden"><header className="border-b border-line px-4 py-3"><h2 className="text-[13px] font-semibold">{t("dashboard.reception.frontDeskActions")}</h2></header><div className="grid divide-y divide-line"><Action href="/reception" icon={<Search />} label={t("dashboard.reception.searchOrScan")} detail={t("dashboard.reception.checkVerdict")} /><Action href="/payments" icon={<Banknote />} label={t("dashboard.reception.collectPayment")} detail={`${outstanding.data?.totalItems ?? 0} members with outstanding balances`} /><Action href="/crm/queues" icon={<Users />} label={t("dashboard.reception.trialQueue")} detail={`${trialLeads.data?.totalItems ?? 0} active trial leads`} /><Action href="/payments/shifts" icon={<Banknote />} label={shift.data ? "Review current cash shift" : "Open a cash shift"} detail={shift.data ? `Opened by ${shift.data.shift.openedByName}` : "Required before recording cash"} /></div></section></div>
  </div>;
}
