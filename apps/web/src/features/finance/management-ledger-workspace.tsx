        ...
        <form onSubmit={submit}>
          <DialogBody className="space-y-4">
            ...
            <div className="rounded-md border border-line-2 bg-sunken/20 p-3">
              <div className="mb-2 flex items-center justify-between gap-2"><div><p className="context-label">Journal lines</p><p className="text-[11.5px] text-ink-3">Enter a debit and credit in {currency}; the server enforces balance.</p></div><Badge variant="outline">{lines.length} lines</Badge></div>
  ...
  return (
    <Table>
      ...
      <TableBody>{entries.map((entry) => <TableRow key={entry.id} className={cn(selectedId === entry.id && "bg-sunken/50")}>
        <TableCell className="whitespace-nowrap"><button type="button" className="text-start font-medium underline-offset-2 hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ink" onClick={() => onSelect(entry.id)} aria-label={`View journal ${entry.memo}`}><DateText iso={entry.postingDate} /></button><p className="font-mono text-[10.5px] text-ink-3">{entry.id.slice(0, 8)}</p></TableCell>
