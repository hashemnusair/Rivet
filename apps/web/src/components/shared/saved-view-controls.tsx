...
import { Bookmark, BookmarkPlus, Copy, Pencil, Trash2 } from "lucide-react";
...
export function SavedViewControls({ surface, state, onApply, hasExplicitState = false }: { surface: SavedViewSurface; state: Record<string, unknown>; onApply: (state: Record<string, unknown>) => void; hasExplicitState?: boolean }) {
  ...
  return (
    <div className="flex items-center gap-1.5">
      ...
      <Button type="button" size="sm" variant="secondary" onClick={() => openDialog("create")}><BookmarkPlus /> Save</Button>
      {selected ? <Button type="button" size="icon" variant="ghost" aria-label={`Update saved view ${selected.name}`} onClick={() => openDialog("edit")}><Pencil /></Button> : null}
      {selected ? <Button type="button" size="icon" variant="ghost" aria-label={`Duplicate saved view ${selected.name}`} onClick={() => openDialog("duplicate")}><Copy /></Button> : null}
