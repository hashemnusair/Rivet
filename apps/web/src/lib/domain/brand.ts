/**
 * Tenant brand values are intentionally a closed, data-only contract. The
 * app never accepts CSS text or arbitrary variable names from a gym.
 */
...
export function resolveBrandColor(paletteKey: unknown, primaryColor: unknown): { paletteKey: BrandPaletteKey; primaryColor: string } {
  ...
  return { paletteKey: custom ? key : key, primaryColor: custom ?? BRAND_PALETTE_PRESETS[key] };
}
