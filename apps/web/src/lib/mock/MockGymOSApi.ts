...
import { buildCsvDocument, buildSectionedCsvDocument, exportList, exportStatusLabel, formatExportDateTime, formatMinorUnits, type CsvValue } from "@/lib/exports/csv";
  ...
  requestExport(input: import("@/lib/domain/qol").ExportRequestInput): Promise<import("@/lib/domain/qol").ExportJob> {
    return this.respond(() => {
      ...
      if (input.kind === "members") {
        ...
        headers = ["Member number", "Full name", "Arabic name", "Phone", "Email", "Gender", "Status", "Current plan", "Membership ends", "Outstanding amount", "Currency", "Home branch", "Preferred language", "Marketing consent", "Tags", "Created", "RIVET member ID"];
        rows = this.db.members.filter((member) => (!requestedBranchId || member.homeBranchId === requestedBranchId) && matches([member.fullName, member.fullNameAr, member.phone, member.email, member.memberNumber])).map((member) => {
          ...
          return [member.memberNumber, member.fullName, member.fullNameAr, member.phone, member.email, exportStatusLabel(member.gender), exportStatusLabel(member.status), plan?.name, membership?.endDate, formatMinorUnits(this.outstandingForMember(member.id).amount, this.db.organization.currency), this.db.organization.currency, branches.get(member.homeBranchId), exportStatusLabel(member.preferredLanguage), exportStatusLabel(member.marketingPreference?.status ?? (member.marketingOptIn ? "explicit_opt_in" : "explicit_opt_out")), exportList(member.tags), formatExportDateTime(member.createdAt, TZ), member.id];
        });
      } else if (input.kind === "leads") {
        title = "CRM leads";
        headers = ["Full name", "Phone", "Email", "Branch", "Stage", "Source", "Owner", "Expected value", "Currency", "Next follow-up", "Lost reason", "Created", "Updated", "RIVET lead ID"];
        rows = this.db.leads.filter((lead) => (!requestedBranchId || lead.branchId === requestedBranchId) && matches([lead.fullName, lead.phone, lead.email])).map((lead) => [lead.fullName, lead.phone, lead.email, branches.get(lead.branchId), exportStatusLabel(lead.stage), exportStatusLabel(lead.source), lead.ownerId ? users.get(lead.ownerId) : "Unassigned", lead.expectedValue ? formatMinorUnits(lead.expectedValue.amount, lead.expectedValue.currency) : "", lead.expectedValue?.currency, formatExportDateTime(lead.nextFollowUpAt, TZ), lead.lostReason, formatExportDateTime(lead.createdAt, TZ), formatExportDateTime(lead.updatedAt, TZ), lead.id]);
      } else if (input.kind === "payments") {
        title = "Payment ledger";
        headers = ["When", "Member", "Member number", "Branch", "Receipt number", "Transaction type", "Payment method", "Amount", "Currency", "Status", "Refunded amount", "Recorded by", "External reference", "Refund reason", "Void reason", "RIVET transaction ID"];
        rows = this.db.payments.filter((payment) => (!requestedBranchId || payment.branchId === requestedBranchId) && matches([payment.receiptNumber, payment.externalReference, members.get(payment.memberId)?.fullName])).map((payment) => [formatExportDateTime(payment.occurredAt, TZ), members.get(payment.memberId)?.fullName, members.get(payment.memberId)?.memberNumber, branches.get(payment.branchId), payment.receiptNumber, exportStatusLabel(payment.type), exportStatusLabel(payment.method), formatMinorUnits(payment.amount.amount, payment.amount.currency), payment.amount.currency, exportStatusLabel(payment.status), payment.refundedAmount ? formatMinorUnits(payment.refundedAmount.amount, payment.refundedAmount.currency) : "", payment.collectedByName, payment.externalReference, payment.refundReason, payment.voidReason, payment.id]);
      } else if (input.kind === "membership_liabilities") {
        title = "Outstanding member balances";
        headers = ["Member", "Member number", "Description", "Issued", "Due", "Total", "Paid", "Outstanding", "Currency", "Status", "RIVET charge ID"];
        rows = this.db.charges.filter((charge) => charge.outstandingAmount.amount > 0 && (!requestedBranchId || members.get(charge.memberId)?.homeBranchId === requestedBranchId) && matches([charge.description, members.get(charge.memberId)?.fullName])).map((charge) => [members.get(charge.memberId)?.fullName, members.get(charge.memberId)?.memberNumber, charge.description, charge.issueDate, charge.dueDate, formatMinorUnits(charge.total.amount, charge.total.currency), formatMinorUnits(charge.paidAmount.amount, charge.paidAmount.currency), formatMinorUnits(charge.outstandingAmount.amount, charge.outstandingAmount.currency), charge.total.currency, exportStatusLabel(charge.status), charge.id]);
      } else if (input.kind === "audit") {
        title = "Audit log";
        headers = ["When", "Branch", "Recorded by", "Role", "Category", "Action", "Record type", "Record", "Summary", "Reason", "Approval status", "RIVET audit ID"];
        rows = this.db.audits.filter((event) => (!requestedBranchId || !event.branchId || event.branchId === requestedBranchId) && matches([event.actorName, event.action, event.entityLabel, event.summary])).map((event) => [formatExportDateTime(event.occurredAt, TZ), event.branchId ? branches.get(event.branchId) : "Organization-wide", event.actorName, exportStatusLabel(event.actorRole), exportStatusLabel(event.category), exportStatusLabel(event.action), exportStatusLabel(event.entityType), event.entityLabel, event.summary, event.reason, exportStatusLabel(event.approvalStatus), event.id]);
      } else if (input.kind === "personal_training") {
        title = "Personal training package orders";
        headers = ["Member", "Member number", "Package", "Sessions purchased", "Total price", "Currency", "Status", "Paid", "Refunded sessions", "Refunded amount", "Created", "Updated", "RIVET PT order ID"];
  ...
  requestMemberPersonalDataExport(idempotencyKey: string): Promise<import("@/lib/domain/qol").ExportJob> {
    return this.respond(() => {
      ...
      const content = buildSectionedCsvDocument({
        ...
        sections: [
          { title: "Profile", headers: ["Field", "Value"], rows: [["Full name", profile?.name], ["Arabic name", profile?.nameAr], ["Email", profile?.email], ["Phone", profile?.phone], ["Date of birth", profile?.dateOfBirth], ["Gender", exportStatusLabel(profile?.gender)], ["Preferred language", exportStatusLabel(profile?.preferredLanguage)], ["Address", profile?.addressLine1], ["City", profile?.city], ["Emergency contact", profile?.emergencyContactName], ["Emergency relationship", profile?.emergencyContactRelationship], ["Emergency phone", profile?.emergencyContactPhone], ["RIVET profile ID", profile?.id ?? this.activeCustomerId]] },
          { title: "Memberships", headers: ["Gym", "Branch", "Member number", "Plan", "Status", "Starts", "Ends", "Balance", "Currency", "Last check-in", "RIVET membership ID"], rows: memberships.map((membership) => [membership.gymName ?? this.db.organization.name, membership.branchName, membership.memberNumber, membership.planName, exportStatusLabel(membership.status), membership.startDate, membership.endDate, formatMinorUnits(membership.balanceMinor, this.db.organization.currency), this.db.organization.currency, formatExportDateTime(membership.lastCheckInAt, TZ), membership.id]) },
          { title: "Charges and balances", headers: ["Description", "Issued", "Due", "Total", "Paid", "Outstanding", "Currency", "Status", "RIVET charge ID"], rows: charges.map((charge) => [charge.description, charge.issueDate, charge.dueDate, formatMinorUnits(charge.total.amount, charge.total.currency), formatMinorUnits(charge.paidAmount.amount, charge.paidAmount.currency), formatMinorUnits(charge.outstandingAmount.amount, charge.outstandingAmount.currency), charge.total.currency, exportStatusLabel(charge.status), charge.id]) },
          { title: "Payments and refunds", headers: ["Gym", "Branch", "Receipt", "When", "Type", "Method", "Amount", "Currency", "Status", "Explanation", "RIVET transaction ID"], rows: transactions.map((transaction) => [transaction.gymName, transaction.branchName, transaction.receiptNumber, formatExportDateTime(transaction.occurredAt, TZ), exportStatusLabel(transaction.type), exportStatusLabel(transaction.method), formatMinorUnits(transaction.amount.amount, transaction.amount.currency), transaction.amount.currency, exportStatusLabel(transaction.status), transaction.explanation, transaction.id]) },
          { title: "Check-ins", headers: ["Gym", "Branch", "When", "Result", "Recorded by", "RIVET check-in ID"], rows: visits.map((visit) => [visit.gym, visit.branchName, formatExportDateTime(visit.occurredAt, TZ), exportStatusLabel(visit.decision), visit.checkedInByName, visit.id]) },
          { title: "Activity timeline", headers: ["Gym", "When", "Type", "Title", "Detail", "RIVET event ID"], rows: activity.map((event) => [event.gym, formatExportDateTime(event.occurredAt, TZ), exportStatusLabel(event.type), event.title, event.detail, event.id]) },
          { title: "Trial bookings", headers: ["Gym", "Branch", "Date", "Time", "Goal", "Status", "RIVET trial ID"], rows: trials.map((booking) => [booking.gymId, booking.branchId, booking.preferredDate, booking.preferredTime, booking.goal, exportStatusLabel(booking.status), booking.id]) },
          { title: "Marketing preference history", headers: ["Changed at", "Status", "Opted in", "Source", "Wording version"], rows: preferenceHistory.map((preference) => [formatExportDateTime(preference.changedAt, TZ), exportStatusLabel(preference.status), preference.optedIn, exportStatusLabel(preference.source), preference.wordingVersion]) },
        ],
      });
