...
export default function MemberImportPage() {
  ...
  return <div className="space-y-5">
    ...
    <PageHeader title="Import members" description="Upload the list you already have. RIVET will recognize its columns, check every person, and show exactly what will happen before creating records." actions={<Button asChild variant="secondary"><Link href="/members"><ArrowLeft /> Back to members</Link></Button>} />
...
function ImportPreviewRows({ preview, currency }: { preview: MemberImportPreview; currency: string }) {
  return (
    <div className="max-h-[32rem] overflow-y-auto">
      <ul className="divide-y divide-line lg:hidden" aria-label="Import preview rows">
        {preview.rows.map((row) => (
          <li key={row.rowNumber} className="space-y-3 px-4 py-3.5">
            ...
            <dl className="grid grid-cols-2 gap-x-4 gap-y-2 border-t border-line pt-3 text-[12.5px]">
              ...
              <div><dt className="text-ink-3">Opening balance</dt><dd className="mt-0.5 tabular">{row.openingBalanceMinor ? formatMinor(row.openingBalanceMinor, currency) : "—"}</dd></div>
              <div className="col-span-2"><dt className="text-ink-3">Membership</dt><dd className="mt-0.5">{row.planName ? `${row.planName} · ${row.membershipStartDate} to ${row.membershipEndDate}${row.freezeStartDate ? " · frozen" : ""}` : "Profile only"}</dd></div>
            </dl>
      ...
      <table className="hidden w-full min-w-[960px] text-start text-[12.5px] lg:table">
        ...
        <tbody className="divide-y divide-line">{preview.rows.map((row) => <tr key={row.rowNumber}><td className="px-5 py-3 font-mono text-ink-3">{row.rowNumber}</td><td className="px-3 py-3"><p className="font-medium text-ink">{row.fullName || "—"}</p><p className="mt-0.5 text-[12px] text-ink-3">{row.email || "No email"}</p></td><td className="px-3 py-3 text-ink-2" dir="ltr">{row.phone || "—"}</td><td className="px-3 py-3 text-ink-2">{row.planName ? <><p className="font-medium text-ink">{row.planName}</p><p className="mt-0.5 text-[12px] text-ink-3">{row.membershipStartDate} → {row.membershipEndDate}{row.freezeStartDate ? " · frozen" : ""}</p></> : "Profile only"}</td><td className="px-3 py-3 tabular text-ink-2">{row.openingBalanceMinor ? formatMinor(row.openingBalanceMinor, currency) : "—"}</td><td className="px-5 py-3"><ImportRowStatus row={row} />{row.errors.length ? <span className="ms-2 text-[12px] text-ink-3">{row.errors.join("; ")}</span> : null}</td></tr>)}</tbody>
      </table>
    </div>
  );
}
...
function ImportRowStatus({ row }: { row: MemberImportPreview["rows"][number] }) {
  const label = row.status === "valid" ? "Ready" : row.status === "committed" ? "Imported" : row.status === "duplicate" ? "Duplicate" : row.status === "invalid" ? "Needs attention" : "Skipped";
