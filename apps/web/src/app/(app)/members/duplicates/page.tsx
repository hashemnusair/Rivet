...
export default function DuplicateMembersPage() {
  ...
  const cases = useApiQuery(qk.duplicateCases(status), (api) => api.listDuplicateCases(status));
  ...
  return <div className="space-y-5">
    <PageHeader eyebrow="Data quality" title="Duplicate members" description="Resolve identity collisions without rewriting payments, visits, memberships, or audit history." actions={<Button asChild variant="secondary"><Link href="/members"><ArrowLeft /> Members</Link></Button>} />
    <div className="flex items-center gap-2"><Select value={status} onValueChange={(value) => setStatus(value as DuplicateCaseStatus)}><SelectTrigger sizeVariant="sm" className="w-44" aria-label="Duplicate case status"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="open">Needs review</SelectItem><SelectItem value="ignored">Ignored</SelectItem><SelectItem value="merged">Merged</SelectItem></SelectContent></Select><p className="text-[11.5px] text-ink-3">{cases.data?.length ?? 0} cases</p></div>
    {cases.isLoading ? <div className="grid gap-3 lg:grid-cols-2">{[0, 1, 2, 3].map((item) => <Skeleton key={item} className="h-64" />)}</div> : cases.isError ? <ErrorState onRetry={() => cases.refetch()} /> : !cases.data?.length ? <EmptyState icon={SearchX} title={status === "open" ? "No duplicate records need review" : `No ${status} duplicate cases`} description={status === "open" ? "New exact phone, email, or member-number matches will appear here." : "Change the status filter to review another queue."} /> : <div className="grid gap-3 lg:grid-cols-2">{cases.data.map((item) => <article key={item.id} className="panel p-4"><div className="flex items-start justify-between gap-3"><div><p className="eyebrow">{item.confidence} match</p><h2 className="mt-1 text-[14px] font-semibold">{item.primary.fullName} ↔ {item.candidate.fullName}</h2><p className="mt-1 text-[11.5px] text-ink-3">Matched on {item.reasons.map((reason) => reason.replaceAll("_", " ")).join(", ")}</p></div><span className="rounded-full border border-line px-2 py-1 text-[10.5px] font-medium capitalize">{item.status}</span></div><div className="mt-4 grid grid-cols-2 gap-3"><MemberSummary member={item.primary} branchName={branchName(item.primary.homeBranchId)} /><MemberSummary member={item.candidate} branchName={branchName(item.candidate.homeBranchId)} /></div>{item.resolutionReason ? <p className="mt-3 rounded-md bg-sunken px-3 py-2 text-[11.5px] text-ink-2">Decision: {item.resolutionReason}</p> : null}{item.status === "open" ? <div className="mt-4 flex justify-end gap-2"><Button size="sm" variant="ghost" onClick={() => openCase(item, "ignore")}>Not a duplicate</Button><Button size="sm" onClick={() => openCase(item, "merge")}><GitMerge /> Review merge</Button></div> : null}</article>)}</div>}

    <Dialog open={Boolean(active)} onOpenChange={(open) => { if (!open) setActive(undefined); }}><DialogContent className="max-w-3xl"><DialogHeader><DialogTitle>{mode === "merge" ? "Merge duplicate member records" : "Ignore duplicate suggestion"}</DialogTitle><DialogDescription>{mode === "merge" ? "Choose the surviving identity and the value to retain for each editable profile field. Operational history stays on its original immutable record and is combined when viewed." : "Explain why these are different people so the decision remains supportable."}</DialogDescription></DialogHeader>{active ? <DialogBody className="space-y-4">{mode === "merge" ? <><label className="grid gap-1.5 text-[12.5px] font-medium">Surviving member<Select value={survivorId} onValueChange={(value) => { setSurvivorId(value); setFieldSources(Object.fromEntries(FIELD_ROWS.map((field) => [field.key, value]))); }}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value={active.primary.id}>{active.primary.fullName} · {active.primary.memberNumber}</SelectItem><SelectItem value={active.candidate.id}>{active.candidate.fullName} · {active.candidate.memberNumber}</SelectItem></SelectContent></Select></label><div className="overflow-hidden rounded-lg border border-line"><div className="grid grid-cols-[130px_1fr_1fr] border-b border-line bg-sunken px-3 py-2 text-[10.5px] font-semibold uppercase tracking-wider text-ink-3"><span>Field</span><span>{active.primary.memberNumber}</span><span>{active.candidate.memberNumber}</span></div>{FIELD_ROWS.map((field) => <div key={field.key} className="grid grid-cols-[130px_1fr_1fr] items-center border-b border-line px-3 py-2 last:border-0"><span className="text-[11.5px] font-medium">{field.label}</span>{[active.primary, active.candidate].map((member) => <label key={member.id} className="flex min-w-0 items-center gap-2 text-[12px]"><input type="radio" name={field.key} checked={fieldSources[field.key] === member.id} onChange={() => setFieldSources((current) => ({ ...current, [field.key]: member.id }))} /><span className="truncate">{fieldValue(member, field.key, branchName)}</span></label>)}</div>)}</div><div className="grid gap-2 rounded-lg border border-signal/30 bg-signal-bg/20 p-3 text-[11.5px] text-ink-2 sm:grid-cols-3"><HistoryFact label="Memberships retained" value={active.primary.membershipCount + active.candidate.membershipCount} /><HistoryFact label="Visits retained" value={active.primary.visitCount + active.candidate.visitCount} /><HistoryFact label="Timeline events retained" value={active.primary.timelineCount + active.candidate.timelineCount} /></div><p className="flex gap-2 text-[11.5px] text-ink-2"><ShieldCheck className="size-4 shrink-0 text-signal-deep" />The archived identity keeps a link to the survivor. The immutable merge audit contains both snapshots and a recovery reference.</p></> : null}<label className="grid gap-1.5 text-[12.5px] font-medium">Decision reason<Textarea value={reason} onChange={(event) => setReason(event.target.value)} placeholder={mode === "merge" ? "How did you verify these records belong to one person?" : "Why are these two different people?"} /></label></DialogBody> : null}<DialogFooter><Button variant="secondary" onClick={() => setActive(undefined)}>Cancel</Button><Button variant={mode === "ignore" ? "secondary" : "primary"} disabled={reason.trim().length < 3 || (mode === "merge" && !survivorId)} loading={merge.isPending || ignore.isPending} onClick={() => mode === "merge" ? merge.mutate() : ignore.mutate()}>{mode === "merge" ? <GitMerge /> : null}{mode === "merge" ? "Merge records" : "Ignore suggestion"}</Button></DialogFooter></DialogContent></Dialog>
  </div>;
}

function MemberSummary({ member, branchName }: { member: DuplicateMemberSummary; branchName: string }) { return <section className="rounded-lg border border-line p-3"><p className="font-mono text-[11px] text-ink-3">{member.memberNumber}</p><p className="mt-1 text-[13px] font-semibold">{member.fullName}</p><p className="mt-1 font-mono text-[11px] text-ink-2" dir="ltr">{member.phone}</p><p className="mt-2 text-[11px] text-ink-3">{branchName} · {member.membershipCount} memberships · {member.visitCount} visits</p>{member.balance.amount ? <p className="mt-1 text-[11px] text-warning-deep">Balance <MoneyText money={member.balance} /></p> : null}</section>; }
function HistoryFact({ label, value }: { label: string; value: number }) { return <p><span className="block font-mono text-[16px] font-semibold text-ink">{value}</span>{label}</p>; }
