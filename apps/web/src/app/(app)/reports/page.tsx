...
function ReportStat({ label, value, tone }: { label: string; value: React.ReactNode; tone?: "warning" }) {
  return <div className="border-e border-line px-4 py-3.5 last:border-e-0"><p className="context-label">{label}</p><div className={tone === "warning" ? "mt-1 text-[20px] tabular text-warning-deep" : "mt-1 text-[20px] tabular"}>{value}</div></div>;
}
