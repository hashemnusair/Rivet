...
import { PT_PACKAGE_PRICE_GUIDE, ptPackageSuggestedPriceMinor, ptPackageUnitPriceMinor } from "@/lib/domain/personal-training";
...
export default function PersonalTrainingPage() {
  ...
  return <div className="space-y-5">
    <PageHeader title="Personal training" description="Trainer availability, member credits, package payments, and session outcomes share one audited ledger." actions={can("pt.manage") ? <div className="flex flex-wrap gap-2"><Button variant="secondary" onClick={() => { setEditingTrainer(undefined); setTrainerOpen(true); }}><UserRound /> Trainer</Button><Button onClick={() => { setEditingPackage(undefined); setPackageOpen(true); }}><Plus /> Package</Button></div> : undefined} />
    ...
    <section className="grid gap-5 xl:grid-cols-[1.15fr_.85fr]">
      <div className="panel overflow-hidden">
        <header className="flex items-center justify-between border-b border-line px-5 py-4"><div><p className="context-label">Schedule</p><h2 className="mt-1 text-[17px] font-semibold">Upcoming sessions</h2></div><CalendarClock className="size-5 text-ink-3" /></header>
      ...
      <div className="panel overflow-hidden">
        <header className="border-b border-line px-5 py-4"><p className="context-label">Payment queue</p><h2 className="mt-1 text-[17px] font-semibold">Pending package orders</h2></header>
...
function Metric({ label, value }: { label: string; value: React.ReactNode }) { return <div className="border-b border-line p-4 last:border-b-0 sm:border-e xl:border-b-0"><p className="context-label">{label}</p><div className="mt-2 text-[22px] font-semibold tabular-nums">{value}</div></div>; }
