    ...
    <div className="px-4 py-6 sm:px-6 lg:px-8 lg:py-8" data-print-root>
      <div className="mx-auto max-w-[1600px]">
        <div className="flex flex-wrap items-end justify-between gap-4 print:hidden">
          <div><p className="eyebrow">Studio</p><h1 className="mt-2 text-[30px] font-semibold tracking-tight">Weekly class schedule</h1><p className="mt-2 text-[12.5px] text-ink-2">One fixed timetable, every week. Press an open slot to add a class; right-press a class to edit it.</p></div>
                        ...
                        {items.map((item) => {
                          ...
                          return (
                            <button
                              key={item.id}
                              type="button"
                              onClick={(event) => { event.stopPropagation(); setManageId(item.id); setMemberSearch(""); setMemberResults([]); }}
        ...
        <Dialog open={Boolean(editor)} onOpenChange={(open) => { if (!open && !save.isPending) setEditor(undefined); }}>
          <DialogContent className="max-w-xl">
            ...
            {editor ? (
              <DialogBody className="grid gap-4">
                <div className="grid gap-4 sm:grid-cols-2">
                  ...
                  <label className="grid gap-1.5 text-[12px] font-medium">Coach<select className="h-10 rounded-md border border-line-2 bg-surface px-3 text-[13px]" value={editor.coachId} onChange={(event) => setEditor({ ...editor, coachId: event.target.value })}><option value="">No coach assigned</option>{coaches.map((coach) => <option key={coach.id} value={coach.id}>{coach.name}</option>)}</select></label>
            ...
            <DialogFooter>
              ...
              <Button variant="signal" loading={save.isPending} disabled={!editor?.name.trim() || editor?.uploading} onClick={() => save.mutate()}><Check /> Save class</Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
        ...
        <Dialog open={coachesOpen} onOpenChange={setCoachesOpen}>
          <DialogContent className="max-w-md">
            ...
            <DialogBody className="grid gap-3">
              ...
              <CoachForm onSubmit={(input) => upsertCoach.mutate(input)} pending={upsertCoach.isPending} />
            </DialogBody>
            <DialogFooter>
