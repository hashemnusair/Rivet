...
export default function SubscriptionsPage() {
  ...
  return <div className="px-4 py-6 sm:px-6 lg:px-8 lg:py-8"><div className="mx-auto max-w-[1480px]">
    ...
    <section className="mt-7 grid gap-3 sm:grid-cols-3"><Kpi label="Active MRR" value={`JD ${(customerGyms.filter((gym) => gym.subscriptionStatus === "active").reduce((total, gym) => total + (plans.find((plan) => plan.name === gym.rivetPlan)?.priceMinor ?? 0), 0) / 1000).toFixed(3)}`} detail={`${customerGyms.filter((gym) => gym.subscriptionStatus === "active").length} customer accounts`} icon={<BadgeDollarSign/>}/><Kpi label="Trial pipeline" value={`JD ${(customerGyms.filter((gym) => gym.subscriptionStatus === "trial").reduce((total, gym) => total + (plans.find((plan) => plan.name === gym.rivetPlan)?.priceMinor ?? 0), 0) / 1000).toFixed(3)}`} detail={`${customerGyms.filter((gym) => gym.subscriptionStatus === "trial").length} trial account${customerGyms.filter((gym) => gym.subscriptionStatus === "trial").length === 1 ? "" : "s"}`} icon={<Clock3/>}/><Kpi label="Expansion ready" value={`${customerGyms.filter((gym) => { const plan = plans.find((item) => item.name === gym.rivetPlan); return Boolean(plan && (gym.memberCount >= plan.members * 0.8 || gym.branchCount >= plan.branches)); }).length} gyms`} detail="Near branch or member limits" icon={<CircleAlert/>}/></section>
    <div className="mt-5 grid gap-5 xl:grid-cols-[1.35fr_.8fr]">
      <section className="overflow-x-auto border border-line bg-surface"><div className="border-b border-line px-5 py-4"><p className="eyebrow">Customer plans</p><h2 className="mt-1 text-[17px] font-semibold">Current subscriptions</h2></div><table className="w-full min-w-[720px] text-start"><thead><tr className="border-b border-line bg-sunken text-start font-mono text-[8px] uppercase tracking-[.1em] text-ink-3"><th className="px-5 py-3 font-medium">Gym</th><th className="px-4 py-3 font-medium">Plan</th><th className="px-4 py-3 font-medium">Usage</th><th className="px-4 py-3 font-medium">Next bill</th><th className="px-4 py-3 font-medium">Status</th><th/></tr></thead><tbody>{customerGyms.map((gym)=><tr key={gym.id} className="border-b border-line last:border-b-0"><td className="px-5 py-4"><p className="text-[12.5px] font-semibold">{gym.name}</p><p className="mt-1 text-[9.5px] text-ink-3">{gym.branchCount} branch{gym.branchCount>1?"es":""}</p></td><td className="px-4 py-4 text-[11.5px]">{gym.rivetPlan}</td><td className="px-4 py-4"><div className="w-28"><div className="h-1 bg-line"><span className="block h-full bg-signal" style={{width:`${Math.min(92,28+gym.memberCount/30)}%`}}/></div><p className="mt-1.5 text-[8.5px] text-ink-3">{gym.memberCount.toLocaleString()} members</p></div></td><td className="px-4 py-4 text-[11px]">{gym.subscriptionStatus==="trial"?"5 Aug 2026":"18 Aug 2026"}</td><td className="px-4 py-4"><span className={gym.subscriptionStatus==="active"?"rounded-full bg-success-bg px-2 py-1 font-mono text-[7.5px] uppercase text-success":gym.subscriptionStatus==="suspended"?"rounded-full bg-danger-bg px-2 py-1 font-mono text-[7.5px] uppercase text-danger":"rounded-full bg-info-bg px-2 py-1 font-mono text-[7.5px] uppercase text-info"}>{gym.subscriptionStatus}</span></td><td className="px-4 py-4"><Button asChild variant="ghost" size="icon-sm"><Link href={`/platform/gyms/${gym.id}`} aria-label={`Open ${gym.name}`}><ArrowRight/></Link></Button></td></tr>)}</tbody></table></section>
