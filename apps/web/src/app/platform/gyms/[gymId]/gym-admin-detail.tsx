...
export default function GymAdminDetail({gymId}:{gymId:string}){
  ...
  const [isPublic, setIsPublic] = useState(directoryGym?.isPublic ?? true);
  useEffect(() => {
    setGym(directoryGym);
    if (directoryGym) {
      setStatus(directoryGym.subscriptionStatus);
      setPlan(directoryGym.rivetPlan);
      setIsPublic(directoryGym.isPublic ?? true);
    }
  }, [directoryGym]);
  ...
  return <div className="px-4 py-6 sm:px-6 lg:px-8 lg:py-8"><div className="mx-auto max-w-[1480px]">
    ...
    <div className="mt-5 grid gap-5 xl:grid-cols-[1.4fr_.8fr]">
      ...
      <div className="grid gap-5"><section className="border border-line bg-surface p-5"><p className="eyebrow">Account owner</p><h2 className="mt-2 text-[17px] font-semibold">Dana Al-Khatib</h2><div className="mt-5 grid gap-3 text-[11.5px] text-ink-2"><p className="flex items-center gap-2"><Mail className="size-3.5 text-ink-3"/>dana@{gym.id}.jo</p><p className="flex items-center gap-2"><Phone className="size-3.5 text-ink-3"/>+962 79 400 1188</p></div><Button variant="secondary" size="sm" className="mt-5">Contact owner</Button></section><section className="night-surface bg-night p-5 text-night-ink"><p className="eyebrow-night">Subscription</p><p className="mt-4 text-[25px] font-semibold">JD {gym.rivetPlan==="Starter"?79:gym.rivetPlan==="Growth"?149:249}.000</p><p className="mt-1 text-[10.5px] text-night-ink-3">Renews 18 August 2026 · Visa •••• 4041</p><div className="mt-5 flex gap-2"><Button variant="night-outline" size="sm">Change plan</Button><Button variant="night-ghost" size="sm">Invoices</Button></div></section></div>
    </div>
    <section className="mt-5 border border-line bg-surface"><div className="border-b border-line px-5 py-4"><p className="eyebrow">Account activity</p><h2 className="mt-1 text-[17px] font-semibold">Platform timeline</h2></div><div className="grid divide-y divide-line md:grid-cols-3 md:divide-x md:divide-y-0">{[["Today · 13:42","Staff session opened","Dana signed into the Abdoun branch."],["28 Jul · 09:10","Invoice paid","RV-1041 paid automatically by Visa."],["24 Jul · 17:21","Plan limits reviewed","Member capacity at 42% of allowance."]].map(([time,title,detail])=><div key={title} className="p-5"><p className="font-mono text-[8px] uppercase tracking-[.1em] text-ink-3">{time}</p><p className="mt-3 text-[12.5px] font-semibold">{title}</p><p className="mt-1 text-[10.5px] leading-relaxed text-ink-3">{detail}</p></div>)}</div></section>
  </div></div>;
}
function Stat({label,value,detail}:{label:string;value:string;detail:string}){return <div className="border border-line bg-surface p-5"><p className="font-mono text-[8px] uppercase tracking-[.11em] text-ink-3">{label}</p><p className="mt-3 text-[23px] font-semibold">{value}</p><p className="mt-2 text-[10px] capitalize text-ink-3">{detail}</p></div>}
function Usage({icon,label,value}:{icon:React.ReactNode;label:string;value:string}){return <div className="bg-surface p-4"><span className="text-ink-3 [&_svg]:size-3.5">{icon}</span><p className="mt-4 font-mono text-[7.5px] uppercase tracking-[.1em] text-ink-3">{label}</p><p className="mt-1 text-[12px] font-semibold">{value}</p></div>}
