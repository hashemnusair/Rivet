    ...
    <main>
      <section className="relative overflow-hidden bg-cover bg-center px-5 py-14 text-white sm:px-8 lg:px-12 lg:py-20" style={{ backgroundColor: gym.accent, backgroundImage: gym.cover?.url ? `linear-gradient(rgb(0 0 0 / .58), rgb(0 0 0 / .58)), url(${gym.cover.url})` : undefined }}>
        ...
        <div className="relative mx-auto max-w-[1344px]">
          <Link href="/customer/discover" className="inline-flex items-center gap-2 text-[12px] font-medium text-white/75 hover:text-white"><ArrowLeft className="size-3.5" /> All gyms</Link>
          <div className="mt-12 grid gap-10 lg:grid-cols-[1fr_0.45fr] lg:items-end">
            <div><div className="flex items-center gap-3"><span className="size-12 rounded-full border border-white/50 bg-cover bg-center" role="img" aria-label={`${gym.name} logo`} style={{ backgroundColor: gym.accent, backgroundImage: gym.logo?.url ? `url(${gym.logo.url})` : undefined }} /> <p className="text-[12px] font-medium text-white/70">{gym.category} · {gym.city}</p></div><h1 className="marketing-display mt-4 max-w-5xl text-[clamp(4rem,8vw,8rem)] leading-[0.87]">{gym.name}</h1><p className="mt-7 max-w-2xl text-[17px] leading-relaxed text-white/85">{gym.tagline}</p></div>
          ...
          <aside id="book-trial" className="h-fit border border-ink bg-surface p-6 shadow-pop lg:sticky lg:top-24">
            {booked ? (
              ...
              <>
                ...
                <p className="mt-4 flex items-center justify-center gap-2 text-[12px] text-ink-3"><Clock className="size-3.5" /> The gym controls confirmation and follow-up.</p>
              </>
            )}
          </aside>
        </div>
...
function GymStat({ label, value }: { label: string; value: string }) { return <div className="bg-black/15 p-4"><dt className="text-[12px] font-medium text-white/65">{label}</dt><dd className="mt-2 text-[17px] font-semibold tabular">{value}</dd></div>; }
...
function TrialField({ label, error, children }: { label: string; error?: string; children: React.ReactNode }) { return <label className="block"><span className="mb-1.5 block text-[11.5px] font-medium">{label}</span>{children}{error ? <span className="mt-1 block text-[12px] text-danger">{error}</span> : null}</label>; }
