...
export default function MemberDashboardPage() {
  ...
  return <main className="mx-auto max-w-[1080px] px-4 py-7 sm:px-6 lg:px-8 lg:py-10">
    <header className="flex flex-wrap items-start justify-between gap-4">
      <div><p className="context-label">Member home</p><h1 className="mt-1 font-display text-[27px] font-semibold tracking-tight">Hi, {customer.name.split(" ")[0]}</h1><p className="mt-1 text-[13px] text-ink-2">Your gym subscriptions, all in one place.</p></div>
    ...
    <section className="mt-9" aria-labelledby="subscribed-gyms-title">
      <div className="flex items-baseline justify-between gap-3"><div><p className="context-label">Your subscriptions</p><h2 id="subscribed-gyms-title" className="mt-1 text-[19px] font-semibold">Subscribed gyms</h2></div><span className="text-[12px] text-ink-3">{customerMemberships.length} {customerMemberships.length === 1 ? "gym" : "gyms"}</span></div>
