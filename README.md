...
| `GT_PROJECT_ID` | General Translation project ID (server-only; Vercel Production) |
| `GT_API_KEY` | General Translation production API key (server-only; Vercel Production) |
...
Vercel's root directory is `apps/web`. The target is a Next.js server deployment; static export is not supported because Clerk's request proxy needs a server runtime. Do not set `NEXT_PUBLIC_RIVET_DEMO_AUTH` on any deployment. The demo bypass is refused in production builds. The Vercel production build fails before Next.js starts when `NEXT_PUBLIC_CONVEX_URL`, `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY`, `GT_PROJECT_ID`, or `GT_API_KEY` is missing, preventing a public bundle from silently shipping without its identity/data clients or translation configuration.

The repository pins Vercel's application build command to `pnpm build`. That command validates the runtime variables and builds Next.js with Webpack. It does not publish a translation catalog unless a localization release explicitly sets `RIVET_TRANSLATE_BUILD=1`; routine web deployments must not spend GT quota or fail because a catalog is unchanged. Convex schema/function deployment is intentionally separate: run `pnpm convex:deploy` from a trusted operator environment with `CONVEX_DEPLOY_KEY`, then deploy the Next.js application. Do not make Vercel Preview builds depend on a production Convex deploy key. Production also requires the server-only `GT_PROJECT_ID` and `GT_API_KEY` names for the General Translation runtime; see `docs/17_GENERAL_TRANSLATION_INTEGRATION.md` for the exact Vercel scopes, provider setup, and explicit catalog-release step.
