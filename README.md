...
| `GT_PROJECT_ID` | General Translation project ID (server-only; reserved for later re-enable) |
| `GT_API_KEY` | General Translation production API key (server-only; reserved for later re-enable) |
...
Vercel's root directory is `apps/web`. The target is a Next.js server deployment; static export is not supported because Clerk's request proxy needs a server runtime. Do not set `NEXT_PUBLIC_RIVET_DEMO_AUTH` on any deployment. The demo bypass is refused in production builds. While General Translation is paused, the Vercel production build validates only `NEXT_PUBLIC_CONVEX_URL`, `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY`, and `NEXT_PUBLIC_DATA_MODE=convex`; GT variables are not required.

The repository pins Vercel's application build command to `pnpm build`. During the temporary translation pause, that command validates the runtime variables and builds Next.js with Webpack; it does not invoke `gtx-cli` or require GT credentials. Convex schema/function deployment is intentionally separate: run `pnpm convex:deploy` from a trusted operator environment with `CONVEX_DEPLOY_KEY`, then deploy the Next.js application. Do not make Vercel Preview builds depend on a production Convex deploy key. See `docs/17_GENERAL_TRANSLATION_INTEGRATION.md` for the preserved integration and later re-enable checklist.
