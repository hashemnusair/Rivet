...
- **Pass 4 is implemented and awaiting Hashem's Preview review.** Work began from fetched `origin/main` at `6b190d5` (the Pass 2 closure) and was integrated onto the Pass 3 closure `9caab72` before any commit; the member section tabs adopt the shared underline tab strip approved in Pass 3. Coherent implementation commits are `9f6a78f`, `5a556d6`, `1d2696d` and `816205a` (rebased onto `21788af`, which realigned the Pass 2 retention references). `main` is not pushed; Pass 5 onward is untouched.
