...
Updated 2026-08-10 after production topology verification, the Production Convex deployment, staging write verification, the disposable production onboarding through workspace provisioning, platform-operations controls, reporting hardening, the end-to-end free-trial lifecycle pass, reception verdict-card responsive hardening, CRM offer-delivery truthfulness, facts-backed CRM stage progress, BUG-012/BUG-013 correctness hardening, attributable marketing preferences, the member communication-preference/history slice, the first native realtime member-experience subscription, explicit membership plan changes, the branded invited-owner handoff, the platform tenant-directory visibility fix, historical audit compatibility, resilient public application/discovery states, and stale-data refresh recovery across experience and operational query surfaces. This is the living implementation and release-status handoff. The historical frontend-only pass is preserved separately in `FRONTEND_HANDOFF.md`.
...
Operational queries use a four-second background refresh while in Convex mode. The authenticated member experience and platform application queue now use native, identity-scoped Convex query subscriptions through `GymOSApi.subscribeCustomerExperience` and `subscribePlatformApplications`, with the same snapshot-preserving error handling and safe mock adapter contracts. Refreshes preserve the already-rendered screen instead of replaying loading gates, so cross-browser trial/status changes and application reviews appear without manual reloads or layout flicker. If the shared experience, a realtime subscription, or an active TanStack Query refresh fails after hydration, the last good snapshot remains visible with a non-blocking retry notice; initial hydration/query failures still fail closed. CRM, reception, and payment/shift surfaces remain on bounded polling until their subscriptions are migrated and browser-tested. Offline/reconnect browser coverage and redacted server-side correlation logging remain open.
