...
Operational queries use a four-second background refresh while in Convex mode. The authenticated member experience, platform application queue, and CRM pipeline now use native, identity-scoped Convex query subscriptions through `GymOSApi.subscribeCustomerExperience`, `subscribePlatformApplications`, and `subscribeLeads`, with snapshot-preserving error handling, a CRM refetch fallback, and safe mock adapter contracts. Refreshes preserve the already-rendered screen instead of replaying loading gates, so cross-browser trial/status changes, application reviews, and lead-stage changes appear without manual reloads or layout flicker. If the shared experience, a realtime subscription, or an active TanStack Query refresh fails after hydration, the last good snapshot remains visible with a non-blocking recovery path; initial hydration/query failures still fail closed. CRM detail/tasks, reception, and payment/shift surfaces remain on bounded polling until their subscriptions are migrated and browser-tested. Offline/reconnect browser coverage and redacted server-side correlation logging remain open.
...
- `pnpm test` — 304 tests passed across 37 files. The new five-test `convex-test` suite invokes exported Convex query/mutation handlers with authenticated identities and persisted fixtures, covering subject-only profile resolution, attacker-controlled customer/email/membership/trial identifiers, My Gyms isolation, selected-gym/branch trial routing, private/suspended/inactive targets, staff/platform denial, deactivated users, inactive staff-membership transition, inactive/foreign entry-pass membership denial, and anonymous-to-authenticated non-attachment. Existing adapter, schema, platform, audit, trial lifecycle, finance, automation, CRM, refresh, invitation, mock, component, reception, customer-helper, and member-portal coverage remains green.
