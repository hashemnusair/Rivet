...
Still release-gated: deploying these Convex/schema changes, credentialed two-context realtime checks, the ten independent staging-write journeys, broader money/staff privilege-escalation handler tests under TODO-006, live operational email activation/provider workers, offline/reconnect browser coverage, and any Production mutation. No Production data, deployment, seed, or live automation was touched in this pass.
...
- `pnpm test` — 324 tests passed across 44 files. New exported-handler coverage exercises platform invoices/subscriptions/support, notification routing, automation preview/run/dedupe/retry, customer ownership, reason requirements, immutable audit facts, staff/platform denial, deactivated identities, and non-disclosing cross-tenant notification access. Realtime hook tests cover cache updates, last-good-snapshot retention, and record-key disposal.
- `pnpm test:e2e` — 23 preview journeys passed and 2 trusted Convex journeys were intentionally skipped without their explicit credential switches. Coverage includes the platform subscription draft/cancel regression, scoped gym detail facts, branch-aware dashboard scope, CRM lead capture, truthful draft/manual offer delivery with skipped-trial assertions, explicit membership plan changes, editing a finalized platform application note, and non-overlapping reception verdict regions at desktop and narrow tablet widths.
