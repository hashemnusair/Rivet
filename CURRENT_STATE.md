...
- **Two doors, one identity.** `/login` no longer carries the generic password form: signed out it shows the gym team door (`/login/gym`) and the member door (`/login/member`), with platform administration as a quiet line; signed in it is still the resolver where the Convex role opens the right area (`IdentityPanel`, organization and branch selection unchanged). Every "Sign in" on the public surfaces now names its door: the bar's "Member sign in", the menu's and footer's "Gym sign in", the application's and the next-step section's "Gym sign in", the gym page's "Sign in as a member". `password-sign-in.client.tsx` and its test are retired.
