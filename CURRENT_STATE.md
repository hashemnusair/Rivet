...
- **Pass 7 is implemented locally and awaits Hashem's Preview review; `main` is not pushed.** Work began from fetched `origin/main` at `b86f146` (the Pass 5 closure) after a clean-tree check; the two pre-existing stashes were left untouched. The Pass 6 Settings work is running in parallel in this same checkout, so every Pass 7 commit stages only its own paths and touches no shared component, Settings file or Pass 6 evidence. Scope is exactly the tracker's Pass 7 list: the nine platform console routes, the sign-in portals and invitation route, the gym application, the legacy sign-up and onboarding redirects, the owner's agreement and getting-started pages, the public offer link, the legal pages, and the offline, not-found and error states.
