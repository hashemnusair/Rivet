...
Operational queries and the authenticated member experience use a four-second background refresh while in Convex mode. Refreshes preserve the already-rendered screen instead of replaying loading gates, so cross-browser trial/status changes appear without manual reloads or layout flicker. If the shared experience or an active TanStack Query refresh fails after hydration, the last good snapshot remains visible with a non-blocking retry notice; initial hydration/query failures still fail closed. This polling boundary preserves the typed `GymOSApi` seam; moving selected screens to native Convex subscriptions remains an optimization rather than a correctness requirement. Offline/reconnect browser coverage and redacted server-side correlation logging remain open.
