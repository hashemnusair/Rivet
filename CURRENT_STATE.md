...
Updated 2026-08-11 after Production frontend/backend alignment for `009b1b8`, in addition to the production topology verification, staging write verification, the disposable production onboarding through workspace provisioning, platform-operations controls, reporting hardening, the end-to-end free-trial lifecycle pass, reception verdict-card responsive hardening, CRM offer-delivery truthfulness, facts-backed CRM stage progress, BUG-012/BUG-013 correctness hardening, attributable marketing preferences, the member communication-preference/history slice, native realtime subscriptions for member experience, platform applications, and the CRM pipeline, explicit membership plan changes, the branded invited-owner handoff, the platform tenant-directory visibility fix, historical audit compatibility, resilient public application/discovery states, stale-data refresh recovery across experience and operational query surfaces, the first adversarial customer/trial ownership handler slice for BUG-002/BUG-004, and the engineering/security hardening pass for authorization matrices, reconnect behavior, redacted diagnostics, scheduler dedupe, and channel-level marketing suppression. This is the living implementation and release-status handoff. The historical frontend-only pass is preserved separately in `FRONTEND_HANDOFF.md`.

## Production release alignment — `009b1b8`

- `origin/main` was confirmed at `10d2930` with the approved branch exactly one fast-forward commit ahead. `main` was fast-forwarded and pushed to `009b1b8` without rewriting concurrent work.
- GitHub Actions run `31481872192` passed generated-code validation, typecheck/lint/unit/build, and 23 preview browser journeys. The authenticated workflow-dispatch job was intentionally skipped on the ordinary push; the approved staging authenticated smoke, disposable operational flow with cleanup, two-browser realtime flow, and offline/reconnect flow were already green before this release.
- Vercel Production deployment `dpl_5M3xaECxsTtqxEqcN1NfLNmQ3g2x` became `READY` at `009b1b8`; the errors-only build-log check reported a completed build and Vercel reported no runtime-error clusters or error/fatal logs for the release deployment.
- Convex Production was explicitly selected as `descriptive-meerkat-589`, not Development `fleet-otter-621`. Its required environment-variable names were present, `health:check` returned `ok`, and the documented 8 August 2026 recovery/export point remains the rollback reference. An isolated `convex deploy --dry-run` named the exact Production deployment, completed schema validation, and reported no index deletions.
- The Production Convex deploy then completed from `009b1b8` with no index deletions. The reviewed schema remains additive only: four optional organization subscription fields (`trialEndsAt`, `currentPeriodEndsAt`, `cancelledAt`, and `subscriptionStatusReason`) plus `operationalNotifications` and its public-id/recipient-created/recipient-dedupe indexes. No seed, import, product-data mutation, restore, or saved platform-control action was run.
- Read-only Production verification passed for health, Platform Overview, Gyms, Subscriptions, the empty notification center, and the empty Platform Support queue. `Hashem Test` remained hidden and suspended. Its Restore shortcut showed an explicit unsaved Active draft; the draft was cancelled and the persisted suspended control returned. The persisted cash shift remains closed with JOD 80.000 expected, JOD 80.000 counted, and JOD 0.000 variance; the deployed reconciliation renderer classifies zero variance as `balanced` even when historical approval metadata exists. The latest 20 Convex log events contained no schema, missing-function, authentication, uncaught, or JavaScript-runtime error line.
- The active authenticated browser identity is platform-only, so a separate gym-owner session is still required to visually recheck the owner dashboard and `/automations` operational-email ledger. The Production ledger contains zero operational-email delivery records, so there is no live delivery to claim; sandbox suppression remains the configured/deployed behavior.
...
Still release-gated: deploying these Convex changes, running the credentialed two-context and offline browser checks against isolated staging, the ten independent staging-write journeys, full allow-path money/staff evidence under TODO-006, live operational email activation/provider workers, historical marketing backfill, and any Production mutation. No Production data, deployment, seed, or live automation was touched in this pass.
...
The frontend still uses the established warm paper/ink visual system, Radix-based UI primitives, RTL logical properties, keyboard-friendly reception contract, `PageHeader`/`Gate` patterns, and TanStack Query hooks. The reception verdict summary now uses bounded responsive grid regions so long English/Arabic member names cannot collide with plan, expiry, visits, or balance facts. CRM offers now remain drafts until staff explicitly confirms manual delivery, so the lead stage and “Offer sent” timeline cannot claim an undelivered message; the lead detail stepper also marks skipped trial milestones instead of treating enum position as historical fact. The public trial form waits for preview-session hydration before exposing editable fields, tracks its semantic gym/customer context, and preserves dirty visitor fields when identity or background marketplace data changes, preventing hydration or a late refresh from silently erasing a request before submission. No page makes a direct Convex or `fetch` call.
...
- `pnpm test` — 332 tests passed across 49 files after the engineering/security hardening pass. New exported-handler coverage exercises selected-branch/cross-tenant member and lead access, payment/refund/void gates, shifts, check-in overrides, staff role/branch escalation, immediate/next-renewal plan changes, scheduler dedupe/opt-out suppression/manager notifications, redacted diagnostics, and realtime offline/reconnect behavior. Existing coverage continues to exercise platform invoices/subscriptions/support, notification routing, automation preview/run/retry, customer ownership, reason requirements, immutable audit facts, and deactivated identities.
- `pnpm test:e2e` — 23 preview journeys passed and 3 trusted Convex journeys were intentionally skipped without their explicit credential switches. The new `convex-realtime-two-browser.spec.ts` is staging-only and requires `PLAYWRIGHT_CONVEX_REALTIME=1`, `PLAYWRIGHT_CONVEX_SMOKE=1`, `PLAYWRIGHT_TARGET_CLASSIFICATION=staging`, and a trusted Clerk storage state; it is not allowed to write to Production.
- `pnpm build` — passed on Next.js 16.2.12 across all App Router routes. The first sandboxed attempt could not reach Google Fonts; the network-enabled rerun compiled, typechecked, generated static routes, and finalized successfully.
...
Vercel Production is `READY` from the verified application commit `0eff62a` with `pnpm build`; the later `443bbd7` head is documentation-only. Its live bundle contains the Production Convex URL, a `pk_live_` Clerk key resolving to `clerk.rivetjo.com`, and `https://www.rivetjo.com` as the canonical origin. The Production Convex deployment `descriptive-meerkat-589` has the seven expected variable names, was safely dry-run and deployed from the latest Convex backend state in `e8be5e4`, and returned `status: ok`; the later `406ab4f` and `0eff62a` commits contain frontend subscription/documentation changes and no Convex function or schema changes. GitHub Actions run `31396158524` passed generated-code, static, 299-test unit/component, build, and 22-journey preview-browser gates; only the explicitly `workflow_dispatch`-gated authenticated Clerk job was skipped. Vercel reported no recent Production runtime errors after the live browser checks.
