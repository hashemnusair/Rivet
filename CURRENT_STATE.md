...
## Product craftsmanship pass: decision clarity, navigable rows and honest permission states — 8 September 2026 (branch `polish/rivet-product-ux`)

- **The member record no longer sends reception into a retry loop.** The record's Payments tab called the transaction list, which the server refuses without `reports.financial.read`, so a receptionist saw "Something went wrong … try again" on every visit even though nothing could change the answer. The tab strip now leaves out a tab the role cannot read (the same way the sidebar leaves out the Payments page), a deep link such as `?tab=payments` lands on a permission state that names the permission and points to the paid/unpaid chips and the Timeline's payment events, and the Timeline, Memberships, Payments and Check-ins tabs render `QueryErrorState`, so a server refusal is a permission wall rather than a "your last loaded data is preserved" message that was never true on first load. Authorization is unchanged and stays server-enforced. Unfilled profile fields on the Details panel (email, gender, date of birth, address, emergency contact, source) read "Not recorded" instead of a dash, because the desk may still need to ask for them.
- **The reception verdict is readable at the moment of decision.** Its four facts shared one row from 1280px, which cut "10-Visit Pass" and the expiry to "10-Visit Pa…" / "2026-10-…" exactly where the desk reads them, and the expiry was a raw ISO date beside human copy. The facts now stay two-across until the lane is genuinely wide (`2xl`), wrap instead of truncating, and print the term date as "24 Oct 2026" like the member header the desk opens next. Verified at 1280 and 390px: every cell fits, no horizontal overflow, the 44px Check in button inside the phone viewport.
- **Rows that open a record behave like links.** The Members and Memberships desktop tables navigated only through a row `onClick`, so keyboard users could not reach a record from the table, middle-click and "open in new tab" did nothing, ticking a row's checkbox needed a `stopPropagation` workaround, and selecting a phone number opened the record. The name cell now carries a real `Link` (keyboard focus, new tab, right-click all work), the shared `TableRow` guards its `interactive` click so clicks that land on a nested control (link, checkbox, button, menu item) or that end a text selection are left alone (`rowClickShouldNavigate`, covered by the new `table.test.tsx`), and the Memberships phone row is a `Link` instead of a `<button>` that pushed a route. The happy-path spec's `p.font-medium` / `p.font-mono` reads still resolve.
- **Leads search keeps what you type.** The pipeline still ran its own URL echo: an effect wrote the settled search into the URL and another effect copied the URL back into the box, so a word typed while the previous write was landing was replaced by the older text (the same defect the 7 September pass fixed on Members, Payments and Audit). The page now uses the shared `useUrlSearchText`, `useReplaceSearchParams`, `choiceFromParams` and `pageFromParams`; search, view and page survive refresh and Back/Forward, a settled search drops a stale page, the box gains "Clear search" and a match count, and the list view separates "No leads yet" (New lead) from "No leads match" (Clear search). The pipeline test now uses the same URL-updating navigation double as the memberships test and adds the typing and Back/Forward cases.
- **Follow-ups is called Follow-ups.** The sidebar, command palette and recent-items entry said "Follow-ups" while the page said "Retention", so the selected sidebar item and the page title disagreed on every visit. The page title now matches the navigation vocabulary (its At risk / Renewals tabs and description are unchanged); the renewal panel prints "Ends 24 Oct 2026" instead of a raw ISO date beside the row that already formatted it. `workflow-pass-2.spec.ts` keeps its `pass-2-retention-*` reference names.
- **Verification.** Focused Vitest (pipeline, memberships, reception, queues, dashboard, member header, member tasks, chrome, use-url-state and the new table suite: 64 tests) and zero-warning lint on every changed file. The shared checkout carries a partner agent's in-progress marketing rewrite (`src/components/marketing/**`, `src/app/page.tsx`), which breaks `tsc` and the dev server's CSS compile there, so this pass was verified from a separate worktree that holds `HEAD` plus only these files: `tsc --noEmit` clean, and mock-mode browser checks through a private `web-mock-polish` launch entry (port 3610, `NEXT_DIST_DIR=.next-playwright/polish`, pointed at the worktree): as Reception the member record's tab strip without Payments and the `?tab=payments` permission state, "Not recorded" details, the reception verdict at 1280 and 390px, the Members table (name link, checkbox tick does not navigate, a plain cell does), the Leads search settling to `?q=` with Clear search, and the Follow-ups heading with the matching sidebar item. The Browser pane was hidden for the whole walk, so steps were driven and measured with in-page JavaScript rather than pointer actions and screenshots.
- **Release status and one inherited failure.** Pushed as `b766940` on `polish/rivet-product-ux` and merged with `origin/main` at `0e444e5` (the landing rewrite; only this file overlapped). GitHub Actions [run 34170068470](https://github.com/hashemnusair/Rivet/actions/runs/34170068470) on the merged tip passed the static job (typechecks, lint, unit tests, build) and the Convex generated-code check; the credential-free browser job finished 157 passed / 2 flaky / 14 skipped / 1 failed. The failure is `public-experience.spec.ts › recovers the public network after a preview live-subscription failure`: on the rewritten landing the new `landing-cinematic_header` intercepts pointer events over the experience provider's sticky "Showing the last known RIVET data … Retry" notice, so the Retry click never lands (reproduced locally on the merged tree with the same interception; the notice and button render). This belongs to the marketing stream that shipped `0e444e5` — the header's stacking or the notice's placement needs one deliberate fix there — and it is unrelated to the files in this pass, which touch no marketing or public component. Not merged to `main`; merging is a review step.
- **Scope kept.** No Convex file, adapter, domain type, error code, authorization rule, money behavior or Production data changed; a backend agent owns `apps/web/convex/**` in parallel and nothing here depends on it. The partner marketing work was left untouched and unstaged. Not changed on purpose: the ISO "start → end" term ranges on the Memberships page and member tabs (a consistent ledger convention across those tables), the Follow-ups at-risk search (per-keystroke URL writes, but no echo defect), and the member-portal finance and discover searches (seeded once from the URL, not refilled by Back/Forward; a small follow-up).
