...
- Added `convex-test` coverage for customer ownership plus new platform invoice/subscription/support/notification/automation boundaries. The credential-free gate now passes 332 tests across 49 files, 23 preview Playwright journeys (with 3 credential-gated Convex journeys skipped), both typechecks, zero-warning lint, production build, and `git diff --check`.
...
- Added an adversarial exported-handler authorization matrix for selected-branch and cross-tenant reads, member/lead writes, payments/refunds/voids, shifts, check-in overrides, staff roles, and privilege-escalation attempts. Added explicit immediate and next-renewal plan-change fixture coverage.
- Added redacted Convex correlation logging at the shared domain boundary. Logs contain operation, correlation ID, error name, and safe error code only; payloads, identities, provider messages, and secrets are excluded.
