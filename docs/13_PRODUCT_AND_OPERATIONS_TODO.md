...
- [x] 12 September: share currency precision between money inputs/displays and exports, recognize the configured currency code without a colloquial alias, and correct supplier-payment/reversal audit text and overpayment errors in both adapters with JOD/USD regressions.
- [ ] Finish currency formatting for newly written membership, retail, member-payment, PT and cash-shift audit/timeline/notification text in both adapters. Several paths still divide minor units by 1,000 or label amounts JOD regardless of currency; USD 4,000 minor units can be displayed as USD 4.000 instead of USD 40.00. Preserve immutable historical events. The shared money/export precision and supplier-payment paths were corrected on 12 September; backend release remains separate.
...
- Lifecycle verification (2026-09-14): `convex/domain.pt-lifecycle.test.ts` proves credit conservation across payment activation, booking, rescheduling, every cancellation kind, outcomes, refunds, expiry, concurrent slot/final-credit attempts and trainer deactivation; deactivation now archives the profile and the preview activates paid orders. The staged journey was repaired for the current dialogs, realtime from an open trainer page and exact-booking cleanup, and still needs the isolated staging credentials named in `CURRENT_STATE.md` before acceptance is claimed.
- Trainer-journey audit (2026-09-14): the preview adapter now mirrors the Convex trainer scoping and booking rules, the workspace and trainer dashboard name the missing setup step (profile, publication, hours), a trainer can cancel their own session, Convex refuses profiles for unaccepted invitations, and a deactivated account is told so at sign-in. Credential-free coverage lives in `e2e/trainer-journey.spec.ts`; the real Clerk acceptance and the credentialed staging journeys below remain the open acceptance.
