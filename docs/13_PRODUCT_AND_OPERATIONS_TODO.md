...
| TODO-006 code-shaped money/staff matrix | 2026-08-11 | `1f29af3` | Persisted branch-transfer and discount-approval matrices, sale-dialog reason-gate tests, and mock/Convex idempotency parity passed. Local gates passed: 51 files/340 tests, both typechecks, Convex codegen, lint, build, 23 preview E2E passes, and diff check. Credentialed staging run `31487507934` passed authenticated smoke, ungated routine membership/payment flow, cleanup, two-browser realtime, and offline/reconnect. Main run `31487723626` passed ordinary checks. Vercel Production `dpl_3CCDV8Kjjr7TJmcp9Cw765vNxEA7` is `READY`; Convex Production `descriptive-meerkat-589` passed exact-target dry-run/deploy and `health:check`. Realistic Production-volume/concurrency proof remains under TODO-007/staging; no Production mutation or seed was run. |
