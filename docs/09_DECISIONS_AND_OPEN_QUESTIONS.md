...
6. **Review applications in the platform console.** Open `/platform/applications`, move a submission to review, approve or reject it with a reason where required, and confirm the decision email plus immutable platform audit event. Workspace provisioning remains a separate protected step until the Clerk organization/invitation workflow is connected.
7. **Provision approved gyms manually until the organization workflow is connected.** Create the tenant and invited owner through the protected platform workflow/Convex owner tooling, then verify the owner can sign in at `/login/gym`. Never grant platform-admin access casually.
8. **Run the trusted smoke.** The isolated Development Clerk session and the four GitHub Actions secrets (`CONVEX_DEPLOY_KEY`, `NEXT_PUBLIC_CONVEX_URL`, `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY`, and `PLAYWRIGHT_CLERK_STORAGE_STATE`) are now configured; the focused smoke passes locally against `fleet-otter-621`. Manually dispatch the `GymOS CI` workflow to record the trusted CI result. This verifies Clerk-to-Convex tenant access rather than only checking that the landing page renders.
...
- GitHub Actions now has all four non-production smoke inputs: `CONVEX_DEPLOY_KEY`, `NEXT_PUBLIC_CONVEX_URL`, `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY`, and `PLAYWRIGHT_CLERK_STORAGE_STATE`. The focused smoke passed locally with the signed-in Development Clerk session; the manual CI dispatch is the remaining recording step.
