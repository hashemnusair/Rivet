...
[Open the protected Vercel Preview](https://rivet-3shzu2ta3-nusairhashem04-gmailcoms-projects.vercel.app/login/gym). GitHub deployment 6296300300 targets Preview and was built by the Vercel Git integration from the pushed review branch `review/ui-workflow-pass-6` at `38107b5`, with synthetic mock data and the existing deployment protection (the earlier deployments 6294324501 from `81c50e2` and 6294439398 from `d6a101e` carry the same application code; the later commits only add test references). Sign in as Omar Al-Khatib (owner); the seeded owner reaches every section. The manager persona cannot open Settings in the seed (no staff or settings permission), so the manager-with-staff-rights case is verified with a component test rather than in the Preview.
...
| Users, Roles & permissions | Before/after; six widths; phone list and role-picker journey; keyboard matrix journey; refreshed Pass 1 desktop Settings reference (`settings-desktop.png`, commit `38107b5`) | Owner; active, invited, deactivated; pending toggle |
...
- The Pass 1 design-system reference `settings-desktop.png` captures the Roles section, which changed on purpose. The old capture still passed within the 4% tolerance (so Playwright's default update mode left it alone); it was rewritten with `--update-snapshots=all` on a quiet macOS checkout in `38107b5`. The dashboard, members and reception captures in the same spec were rewritten by that mode too and restored, because they carry clock-dependent text and drift with the wall clock rather than the design.
