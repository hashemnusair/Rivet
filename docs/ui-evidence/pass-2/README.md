...
- At the time of this pass the mock PT workspace still required `pt.reports.read` while Convex also allowed `pt.schedule.self` and scoped trainers to their own schedule, so the preview could not demonstrate the trainer workspace directly. Resolved on 14 September 2026: the mock mirrors the Convex scoping and `e2e/trainer-journey.spec.ts` walks the trainer workspace in the preview. A component test covers its actual controls against that scoped response, and the existing Convex test verifies the authorization/data boundary. No permission was broadened to make a preview work.
